...
async function decryptJSON(key, payload) {
  const [ivB64, cB64] = payload.split('.');
  const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: b64ToBytes(ivB64) }, key, b64ToBytes(cB64));
...
class App extends React.Component {
  constructor(props){
    ...
    this.state = {
      ...
      filters: { from:'', to:'', accountId:'', categoryId:'', type:'' },
  ...
  componentDidMount(){
    ...
    this._onKey = (e) => { if (e.key === 'Escape' && this.state.modal.open) this.closeModal(); };
  ...
  parseNum(v){
    ...
    if(!s) return 0;
    // Formato it-IT: la virgola è il separatore decimale, il punto le migliaia.
    // Rimuoviamo TUTTI i punti (non solo il primo) prima di convertire la virgola in punto,
    // altrimenti importi come "1.234.567,89" venivano troncati a 1234.567.
  ...
  newAccount(){ this.openModal('account','create',{ name:'', bank:'', type:'Current Account', initialBalance:'', onBudget:true }); }
  editAccount(a){ this.openModal('account','edit',{ id:a.id, name:a.name, bank:a.bank, type:a.type, initialBalance:a.initialBalance, onBudget:a.onBudget!==false }); }
  newTransaction(){
    const firstAcc = this.state.accounts[0];
  ...
  setTxnType(type){
    ...
    if (type==='Transfer'){ data.categoryId=''; if(!data.toAccountId){ const other=this.state.accounts.find(a=>a.id!==data.accountId); data.toAccountId=other?other.id:''; } }
  ...
  saveModal(){
    ...
    if (m.kind==='account'){
      ...
      const payload = { name:d.name.trim(), bank:(d.bank||'').trim()||'—', type:d.type, initialBalance:this.parseNum(d.initialBalance), onBudget: d.onBudget!==false };
  ...
  setAssigned(categoryId, e){ this.store.upsertBudget(this.state.month, categoryId, this.parseNum(e.target.value)); this.refresh(); }
  ...
  monthAgg(mo, accId, catSet){
    ...
    for(const t of this.state.transactions){
      ...
      else if(t.type==='Expense'){ if(catSet && catSet.indexOf(t.categoryId)<0) continue; expense += t.amount||0; }
    }
  ...
  nwAt(mo, accId){
    ...
    let bal = accId ? (this.accById(accId).initialBalance||0) : this.state.accounts.reduce((s,a)=>s+(a.initialBalance||0),0);
    for(const t of this.state.transactions){
      if((t.date||'').slice(0,7) > mo) continue;
      const amt = t.amount||0;
  ...
  catSpentMonth(catId, mo){
    let s=0;
    for(const t of this.state.transactions){ if(t.type==='Expense' && t.categoryId===catId && (t.date||'').slice(0,7)===mo) s += t.amount||0; }
    return s;
  }
  catAssignedMonth(catId, mo){ const b = this.state.budgets.find(x=>x.month===mo && x.categoryId===catId); return b ? (b.assigned||0) : 0; }
  earliestMonth(){
    let min=null;
  ...
  catCarryover(catId, month){
    // Regola envelope budgeting: un saldo POSITIVO si riporta al mese successivo;
    // un saldo NEGATIVO (categoria sovraspesa) non si accumula come debito sulla
    // categoria — si azzera, e quella sovraspesa riduce invece il "Pronto da
    // assegnare" generale (vedi readyToAssign()).
    let mo=this.earliestMonth(); let bal=0; let guard=0;
    while(mo < month && guard<600){ bal = Math.max(0,bal) + this.catAssignedMonth(catId,mo) - this.catSpentMonth(catId,mo); mo=this.addMonth(mo,1); guard++; }
    return Math.max(0, bal);
  }
  // Denaro disponibile in tutti i conti "a budget" considerando solo i movimenti
  // fino alla fine del mese indicato (incluso) — è la base da cui si sottraggono
  // le disponibilità già "prenotate" nelle categorie per ottenere il Pronto da assegnare.
  onBudgetMoneyThroughMonth(month){
    return this.state.accounts.filter(a=>a.onBudget!==false).reduce((sum,acc)=>{
      let b = acc.initialBalance || 0;
      for(const t of this.state.transactions){
  ...
  // Pronto da assegnare = tutto il denaro sui conti a budget MENO quanto è
  // già "prenotato" (disponibilità positiva) nelle categorie. Una categoria
  // sovraspesa (disponibile negativo) non viene sottratta: la sua sovraspesa
  // riduce automaticamente questo totale, com'è corretto che sia.
  readyToAssign(month){
    const cats = this.state.categories.filter(c=>c.type==='Expense');
    const reserved = cats.reduce((s,c)=>{
      const avail = this.catCarryover(c.id, month) + this.catAssignedMonth(c.id, month) - this.catSpentMonth(c.id, month);
      return s + Math.max(0, avail);
    }, 0);
    return this.onBudgetMoneyThroughMonth(month) - reserved;
  }
  ...
  ageOfMoney(dMonths, dAcc){
    ...
    for(const t of this.state.transactions){ if(t.type==='Expense' && inP(t.date) && (!dAcc||t.accountId===dAcc)){ exp+=t.amount||0; active[(t.date||'').slice(0,7)]=1; } }
  ...
  drillMonth(mo){ this.setState({ view:'transactions', drawerOpen:false, filters:{ from:mo+'-01', to:this.lastDayOf(mo), accountId:this.state.dash.accountId, categoryId:'', type:'' } }); }
  drillCategory(cid, fromMo, toMo, type){ this.setState({ view:'transactions', drawerOpen:false, filters:{ from:fromMo+'-01', to:this.lastDayOf(toMo), accountId:this.state.dash.accountId, categoryId:cid, type: type||'Expense' } }); }

  /* ---------- CSV export / import ---------- */
  fmtDate2(iso){ if(!iso) return ''; const p = iso.split('-'); return p[2]+'/'+p[1]+'/'+p[0]; }
  csvNum(v){ v = String(v==null?'':v).replace(/[^0-9.,\-]/g,''); if(!v) return 0; if(v.indexOf(',')>-1){ v = v.replace(/\./g,'').replace(',','.'); } return Math.abs(parseFloat(v)) || 0; }
  ...
  resetData(){ if(window.confirm('Ripristinare i dati d\u2019esempio? Le tue modifiche andranno perse.')){ this.store.reset(); this.setState({ month:this.today().slice(0,7), filters:{from:'',to:'',accountId:'',categoryId:'',type:''} }); this.refresh(); } }

  onFilterChange(e){ const k = e.target.dataset.filter; const filters = Object.assign({}, this.state.filters); filters[k] = e.target.value; this.setState({ filters }); }
  clearFilters(){ this.setState({ filters:{ from:'', to:'', accountId:'', categoryId:'', type:'' } }); }

  /* ---------- render ---------- */
  buildViewModel(){
    const st = this.state;
    const P = { text:'#e7e9ee', muted:'#9297a1', dim:'#6b7079', accent:'#5b8def', green:'#3ecf8e', red:'#f0616d', amber:'#e2b341' };
    ...
    let sidebarStyle;
    if (st.isMobile){
      sidebarStyle = { position:'fixed', top:0, left:0, bottom:0, width:'236px', flexShrink:0, padding:'20px 16px', display:'flex', flexDirection:'column', background:'#0e0f13', borderRight:'1px solid #191c22', zIndex:50, transform: st.drawerOpen ? 'translateX(0)' : 'translateX(-110%)', transition:'transform .25s cubic-bezier(.2,.8,.3,1)', boxShadow: st.drawerOpen ? '0 0 40px rgba(0,0,0,0.6)' : 'none' };
    } else if (collapsed){
      sidebarStyle = { width:'0px', flexShrink:0, padding:'20px 0', display:'flex', flexDirection:'column', background:'#0e0f13', borderRight:'1px solid transparent', overflow:'hidden', opacity:0, transition:'width .22s ease, padding .22s ease, opacity .18s ease', pointerEvents:'none' };
    } else {
      sidebarStyle = { width:'236px', flexShrink:0, padding:'20px 16px', display:'flex', flexDirection:'column', background:'#0e0f13', borderRight:'1px solid #191c22', overflow:'hidden', transition:'width .22s ease, padding .22s ease, opacity .18s ease' };
    }
    const navItem = (active) => ({ display:'flex', alignItems:'center', gap:'11px', width:'100%', padding:'9px 11px', border:'none', borderRadius:'9px', fontSize:'13.5px', fontWeight:600, cursor:'pointer', textAlign:'left', background: active?'#1a1d24':'transparent', color: active?'#e7e9ee':'#7d828c', transition:'background .12s' });
    ...
    const budgetMap = {}; st.budgets.filter(b=>b.month===st.month).forEach(b=> budgetMap[b.categoryId]=b.assigned);
    ...
    const buildBudgetRow = (c) => {
      ...
      const spent = monthTxns.filter(t=>t.type==='Expense' && t.categoryId===c.id).reduce((s,t)=>s+t.amount,0);
      ...
      const barColor = over ? P.red : (target>0 ? (funded?P.green:P.accent) : (assigned===0?'#3a3f49':P.accent));
      let hint='';
      if(target>0){ const need=target-assigned; hint = need>0.005 ? ('Obiettivo '+this.fmtEur(target)+' \u00b7 mancano '+this.fmtEur(need)) : ('Obiettivo '+this.fmtEur(target)+' \u00b7 finanziato'); }
      else if(Math.abs(carry)>0.005){ hint = 'Riporto '+(carry>0?'+':'')+this.fmtEur(carry); }
      return { id:c.id, name:c.name, hint, hasHint:!!hint, hintStyle:{ fontSize:'11.5px', color:(target>0&&!funded)?'#e2b341':'#5a5f68', marginTop:'3px' }, assigned, assignedValue: assigned ? assigned.toFixed(2).replace('.',',') : '', spentText:this.fmtEur(spent), availText:this.fmtEur(avail), availStyle:{ fontFamily:"'JetBrains Mono',monospace", fontWeight:600, fontSize:'13px', color:availColor }, barStyle:{ height:'100%', width:pct+'%', background:barColor, borderRadius:'4px', transition:'width .3s' }, onAssign:(e)=>this.setAssigned(c.id,e) };
    };
    ...
    const budgetGroups = groupNames.map(g => {
      ...
      return { name: g||'Senza gruppo', rows, assignedText:this.fmtEur(gAssigned), isEmpty: rows.length===0 };
    }).filter(gr => !(gr.isEmpty && gr.name==='Senza gruppo'));
    const ready = this.readyToAssign(st.month);
    const readyPositive = ready >= -0.005;
    const readyCardStyle = { padding:'10px 16px', borderRadius:'12px', textAlign:'right', minWidth:'170px', border:'1px solid', background: readyPositive?'rgba(62,207,142,0.09)':'rgba(240,97,109,0.09)', borderColor: readyPositive?'rgba(62,207,142,0.3)':'rgba(240,97,109,0.3)', color: readyPositive?P.green:P.red };
    const readyLabel = readyPositive ? 'Pronto per assegnare' : 'Sovra-assegnato';
    ...
    const spendPalette = ['#5b8def','#3ecf8e','#e2b341','#8b7cf6','#f0616d','#4bb6c9','#e08a4b','#c98bd0','#7d9a4b','#d0576b','#5bbfae','#9297a1'];
    ...
    const dashRangeBtns = [['1m','1M'],['3m','3M'],['6m','6M'],['12m','1A'],['ytd','YTD'],['all','Tutto'],['custom', st.isMobile ? 'Pers.' : 'Personalizzato']].map(x => ({
      key:x[0], label:x[1], onClick:()=>this.setDashRange(x[0]),
      style:{ padding:'6px 12px', border:'none', borderRadius:'7px', fontSize:'12.5px', fontWeight:600, cursor:'pointer', background: dash.range===x[0]?'#5b8def':'transparent', color: dash.range===x[0]?'#fff':'#9297a1', transition:'all .12s', flexShrink:0, whiteSpace:'nowrap' }
    }));
    ...
    const dashSubNavItem = (active) => ({ display:'flex', alignItems:'center', width:'100%', padding:'7px 10px', border:'none', borderRadius:'7px', fontSize:'12.5px', fontWeight: active?600:500, cursor:'pointer', textAlign:'left', background: active?'rgba(91,141,239,0.12)':'transparent', color: active?'#8fb4f5':'#6b7079', transition:'background .12s' });
    ...
    const nwLinePath = nwPts.length>1
      ? nwPts.map((p,i) => (i===0?'M':'L') + ' ' + p[0].toFixed(2) + ' ' + p[1].toFixed(2)).join(' ')
      : (nwPts.length===1 ? ('M 0 ' + nwPts[0][1].toFixed(2) + ' L 100 ' + nwPts[0][1].toFixed(2)) : '');
    const nwAreaPath = nwLinePath + ' L 100 60 L 0 60 Z';
    const nwDots = nwPts.map((p,i) => ({ style:{ position:'absolute', left:p[0]+'%', top:(p[1]/60*100)+'%', width:'7px', height:'7px', marginLeft:'-3.5px', marginTop:'-3.5px', borderRadius:'50%', background:'#5b8def', border:'2px solid #101216' }, title: this.monthAbbr(dMonths[i]) + ': ' + this.fmtEur(nwVals[i]) }));
    ...
    const allocColors = { 'Current Account':'#5b8def','Card':'#8b7cf6','Deposit':'#3ecf8e','Broker':'#e2b341','Pension Fund':'#4bb6c9','Cash':'#9297a1','Crypto':'#e08a4b','Other':'#7d828c' };
    const allocMap={}; st.accounts.forEach(a=>{ const bal=this.accountBalance(a); if(bal>0) allocMap[a.type]=(allocMap[a.type]||0)+bal; });
    const allocTotal=Object.keys(allocMap).reduce((s,k)=>s+allocMap[k],0)||1;
    let allocCum=0;
    const allocSegs=Object.keys(allocMap).sort((a,b)=>allocMap[b]-allocMap[a]).map(k=>{ const val=allocMap[k]; const pv=(val/allocTotal)*100; const seg={ type:k, label:this.typeLabelAcc(k), color:allocColors[k]||'#7d828c', dotStyle:{ width:'9px', height:'9px', borderRadius:'2px', background:allocColors[k]||'#7d828c', flexShrink:0 }, valueText:this.fmtEur(val), pctText:Math.round(pv)+'%', dash:pv.toFixed(2)+' '+(100-pv).toFixed(2), offset:(25-allocCum).toFixed(2) }; allocCum+=pv; return seg; });
    const allocTotalText=this.fmtEur(allocTotal);
    const allocHasData = Object.keys(allocMap).length>0;
    ...
    const byCat = {}; expTxns.forEach(t => { const k=t.categoryId||'__none__'; byCat[k]=(byCat[k]||0)+t.amount; });
    ...
    const ieBars = ieRaw.map(b => { const net=b.income-b.expense; return {
      ...
      netStyle:{ fontFamily:"'JetBrains Mono',monospace", fontSize:'12px', fontWeight:600, color: net>=0?P.green:P.red },
    ...
    const byIncCat = {}; incTxns.forEach(t => { const k=t.categoryId||'__none__'; byIncCat[k]=(byIncCat[k]||0)+t.amount; });
    ...
    const txnRows = txns.map(t => {
      ...
      else if (cat){ catName=cat.name; tagStyle=Object.assign({},tagBase,{ background:'#1c1f26', color:P.muted }); }
      else { catName='Senza categoria'; tagStyle=Object.assign({},tagBase,{ background:'#1c1f26', color:P.dim }); }
      ...
      const clrStyle = clr==='reconciled' ? { width:'8px', height:'8px', borderRadius:'50%', background:'#3ecf8e', flexShrink:0 } : (clr==='cleared' ? { width:'8px', height:'8px', borderRadius:'50%', background:'#5b8def', flexShrink:0 } : { width:'8px', height:'8px', borderRadius:'50%', background:'transparent', border:'1.5px solid #3a3f49', boxSizing:'border-box', flexShrink:0 });
      return { id:t.id, dateLabel:this.fmtDate(t.date), description:t.description, subLabel:sub, hasSub:!!sub, categoryName:catName, tagStyle, accountName:accName, clearedStyle:clrStyle, clearedTitle:clrTitle, amountText: sign + ' ' + this.fmtEur(t.amount), amountStyle:{ fontFamily:"'JetBrains Mono',monospace", fontWeight:600, fontSize:'13px', color:amtColor }, onEdit:()=>this.editTransaction(t), onDelete:()=>this.delTransaction(t) };
    });
    const hasFilters = !!(f.from||f.to||f.accountId||f.categoryId||f.type);
    ...
    const modalTitles = { account: m.mode==='edit'?'Modifica conto':'Nuovo conto', transaction: m.mode==='edit'?'Modifica movimento':'Nuovo movimento', category: m.mode==='edit'?'Modifica categoria':'Nuova categoria', group: m.mode==='edit'?'Modifica gruppo':'Nuovo gruppo', import:'Importa movimenti' };
    const saveLabel = m.kind==='import' ? 'Importa' : 'Salva';
    const accountTypeOptions = this.accountTypes.map(x=>({ value:x[0], label:x[1] }));
    const modalAccountOptions = st.accounts.map(a=>({ value:a.id, label:a.name }));
    ...
    const segBtn = (active) => ({ flex:1, padding:'8px', border:'none', borderRadius:'7px', fontSize:'12.5px', fontWeight:600, cursor:'pointer', background: active?'#5b8def':'transparent', color: active?'#fff':'#9297a1', transition:'all .12s' });
    ...
    const clrSeg = (active,col) => ({ flex:1, padding:'8px', border:'none', borderRadius:'7px', fontSize:'12px', fontWeight:600, cursor:'pointer', background: active?col:'transparent', color: active?'#fff':'#9297a1', transition:'all .12s' });
    const clearedBtn = { uncleared:clrSeg((d.cleared||'uncleared')==='uncleared','#6b7079'), cleared:clrSeg(d.cleared==='cleared','#5b8def'), reconciled:clrSeg(d.cleared==='reconciled','#3ecf8e') };
    ...
    return {
      ...
      onReset:()=>this.resetData(), onLogout:()=>this.logout(),
      ...
      // budget
      monthLabel:this.monthLabel(st.month), onPrevMonth:()=>this.setState({month:this.addMonth(st.month,-1)}), onNextMonth:()=>this.setState({month:this.addMonth(st.month,1)}),
      readyCardStyle, readyLabel, readyText, budgetGroups, budgetEmpty: expenseCatList.length===0,
      // transactions
      ...
      isTransferModal, showCategoryField, accountFieldLabel: isTransferModal?'Conto di origine':'Conto',
  ...
  renderLogin(){
    ...
    return (
      <div style={{minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0c0d10', color:'#e7e9ee', fontFamily:"'Hanken Grotesk',system-ui,-apple-system,sans-serif", padding:'20px'}}>
        <form onSubmit={(e)=>this.submitLogin(e)} style={{width:'100%', maxWidth:'360px', background:'#101216', border:'1px solid #1c1f26', borderRadius:'16px', padding:'28px'}}>
          ...
          <label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600', marginBottom:'14px'}}>
            Utente
            <input type="text" autoCapitalize="none" autoCorrect="off" value={username} onChange={(e)=>this.onAuthUsername(e)} style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', color:'#e7e9ee', fontSize:'16px'}} />
          </label>
          <label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600', marginBottom:'6px'}}>
            Password
            <input type="password" value={password} onChange={(e)=>this.onAuthPassword(e)} style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', color:'#e7e9ee', fontSize:'16px'}} />
          </label>
          <label style={{display:'flex', alignItems:'center', gap:'8px', fontSize:'12.5px', color:'#9297a1', margin:'14px 0 6px', cursor:'pointer'}}>
            <input type="checkbox" checked={remember} onChange={(e)=>this.onAuthRemember(e)} style={{fontSize:'16px', width:'15px', height:'15px', accentColor:'#5b8def'}} />
  ...
  render(){
    ...
    return (
<div style={{display:'flex', minHeight:'100vh', background:'#0c0d10', color:'#e7e9ee', fontFamily:'\'Hanken Grotesk\',system-ui,-apple-system,sans-serif', fontSize:'14px', WebkitFontSmoothing:'antialiased'}}>{(showScrim) ? (<React.Fragment><div onClick={closeDrawer} style={{position:'fixed', inset:'0', background:'rgba(0,0,0,0.55)', zIndex:'40', animation:'fpOverlay .2s ease'}}></div></React.Fragment>) : null}<aside style={sidebarStyle}><div style={{display:'flex', alignItems:'center', gap:'10px', padding:'4px 6px 20px'}}><div style={{width:'30px', height:'30px', borderRadius:'8px', background:'linear-gradient(140deg,#5b8def,#3d6fd6)', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 2px 10px rgba(91,141,239,0.35)'}}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round"><path d="M4 18V9M9 18V5M14 18v-6M19 18v-9" /></svg></div><div style={{lineHeight:'1.1'}}><div style={{fontWeight:'700', fontSize:'14.5px', letterSpacing:'-0.2px'}}>Finanza</div><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'500'}}>Personale</div></div></div><nav style={{display:'flex', flexDirection:'column', gap:'2px'}}><button onClick={goDashboard} style={navStyle.dashboard}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="3" width="8" height="10" rx="1.5" /><rect x="13" y="3" width="8" height="6" rx="1.5" /><rect x="3" y="15" width="8" height="6" rx="1.5" /><rect x="13" y="11" width="8" height="10" rx="1.5" /></svg><span>Dashboard</span></button>{(isDashboard) ? (<React.Fragment><div style={{display:'flex', flexDirection:'column', gap:'1px', margin:'2px 0 6px 14px', paddingLeft:'13px', borderLeft:'1px solid #21242b'}}>{(dashSideItems || []).map((d, dIdx) => (<React.Fragment key={dIdx}><button onClick={d.onClick} style={d.style}>{d.label}</button></React.Fragment>))}</div></React.Fragment>) : null}<button onClick={goBudget} style={navStyle.budget}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="12" r="8" /><circle cx="12" cy="12" r="3" /></svg><span>Budget</span></button><button onClick={goTransactions} style={navStyle.transactions}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><line x1="4" y1="7" x2="20" y2="7" /><line x1="4" y1="12" x2="20" y2="12" /><line x1="4" y1="17" x2="13" y2="17" /></svg><span>Movimenti</span></button><button onClick={goAccounts} style={navStyle.accounts}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="6" width="18" height="12" rx="2" /><line x1="3" y1="10.5" x2="21" y2="10.5" /></svg><span>Conti</span></button><button onClick={goCategories} style={navStyle.categories}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="4" y="4" width="7" height="7" rx="1.5" /><rect x="13" y="4" width="7" height="7" rx="1.5" /><rect x="4" y="13" width="7" height="7" rx="1.5" /><rect x="13" y="13" width="7" height="7" rx="1.5" /></svg><span>Categorie</span></button></nav><div style={{marginTop:'auto', paddingTop:'16px'}}><div style={{background:'#131519', border:'1px solid #21242b', borderRadius:'12px', padding:'14px'}}><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.5px'}}>Patrimonio netto</div><div style={netWorthStyle}>{totalNetWorthText}</div><div style={{fontSize:'11.5px', color:'#6b7079', marginTop:'2px'}}>{accountsCountText}</div></div><button onClick={onReset} style={{width:'100%', marginTop:'10px', padding:'8px', background:'transparent', border:'1px solid #21242b', borderRadius:'9px', color:'#6b7079', fontSize:'12px', fontWeight:'500', cursor:'pointer'}} {...hoverStyle({width:'100%', marginTop:'10px', padding:'8px', background:'transparent', border:'1px solid #21242b', borderRadius:'9px', color:'#6b7079', fontSize:'12px', fontWeight:'500', cursor:'pointer'}, {borderColor:'#f0616d', color:'#f0616d'})}>Ripristina dati d'esempio</button><button onClick={onLogout} style={{width:'100%', marginTop:'8px', padding:'8px', background:'transparent', border:'1px solid #21242b', borderRadius:'9px', color:'#6b7079', fontSize:'12px', fontWeight:'500', cursor:'pointer'}} {...hoverStyle({width:'100%', marginTop:'8px', padding:'8px', background:'transparent', border:'1px solid #21242b', borderRadius:'9px', color:'#6b7079', fontSize:'12px', fontWeight:'500', cursor:'pointer'}, {borderColor:'#2d313a', color:'#e7e9ee'})}>Esci</button></div></aside><main style={{flex:'1', minWidth:'0', display:'flex', flexDirection:'column', height:'100vh', overflow:'hidden'}}><header style={{display:'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center', gap: isMobile ? '12px' : '14px', padding: isMobile ? '14px 16px' : '18px 28px', borderBottom:'1px solid #191c22', flexShrink:'0'}}><div style={{display:'flex', alignItems:'center', gap:'14px'}}><button onClick={onMenu} title={menuTitle} style={{width:'36px', height:'36px', flexShrink:'0', display:'flex', alignItems:'center', justifyContent:'center', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', cursor:'pointer'}} {...hoverStyle({width:'36px', height:'36px', flexShrink:'0', display:'flex', alignItems:'center', justifyContent:'center', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', cursor:'pointer'}, {borderColor:'#2d313a'})}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round"><line x1="4" y1="7" x2="20" y2="7" /><line x1="4" y1="12" x2="20" y2="12" /><line x1="4" y1="17" x2="20" y2="17" /></svg></button><div style={{flex:'1', minWidth:'0'}}><h1 style={{margin:'0', fontSize: isMobile ? '16.5px' : '19px', fontWeight:'700', letterSpacing:'-0.3px'}}>{navTitle}</h1>{isMobile ? null : (<p style={{margin:'2px 0 0', fontSize:'12.5px', color:'#6b7079'}}>{navSubtitle}</p>)}</div></div>{isMobile ? (<button onClick={onPrimary} style={{display:'inline-flex', alignItems:'center', justifyContent:'center', gap:'7px', padding:'10px 15px', background:'#5b8def', border:'none', borderRadius:'9px', color:'#fff', fontSize:'13px', fontWeight:'600', cursor:'pointer', boxShadow:'0 1px 8px rgba(91,141,239,0.3)'}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg><span>{primaryLabel}</span></button>) : (<button onClick={onPrimary} style={{display:'inline-flex', alignItems:'center', gap:'7px', padding:'9px 15px', background:'#5b8def', border:'none', borderRadius:'9px', color:'#fff', fontSize:'13px', fontWeight:'600', cursor:'pointer', boxShadow:'0 1px 8px rgba(91,141,239,0.3)', whiteSpace:'nowrap'}} {...hoverStyle({display:'inline-flex', alignItems:'center', gap:'7px', padding:'9px 15px', background:'#5b8def', border:'none', borderRadius:'9px', color:'#fff', fontSize:'13px', fontWeight:'600', cursor:'pointer', boxShadow:'0 1px 8px rgba(91,141,239,0.3)', whiteSpace:'nowrap'}, {background:'#6f9bf2'})}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg><span>{primaryLabel}</span></button>)}</header><div style={{flex:'1', overflowY:'auto', padding: isMobile ? '14px' : '28px'}}>{/* ============ DASHBOARD ============ */}{(isDashboard) ? (<React.Fragment><div style={{maxWidth:'1080px', margin:'0 auto'}}>{/* Filtro unico: conto + periodo, condiviso da KPI e dalle 4 schede sotto */}<div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:'14px', flexWrap:'wrap', marginBottom:'20px'}}><div style={{display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap'}}><select value={dashAccountId} onChange={onDashAccount} style={{padding:'8px 11px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', fontSize:'16px', fontWeight:'500'}} style-focus="border-color:#5b8def;"><option value="">Tutti i conti</option>{(dashAccountOptions || []).map((opt, optIdx) => (<React.Fragment key={optIdx}><option value={opt.value}>{opt.label}</option></React.Fragment>))}</select><div style={{display:'flex', gap:'2px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', padding:'3px', overflowX:'auto', maxWidth:'100%', WebkitOverflowScrolling:'touch'}}>{(dashRangeBtns || []).map((b, bIdx) => (<React.Fragment key={bIdx}><button onClick={b.onClick} style={b.style}>{b.label}</button></React.Fragment>))}</div>{(dashCustomOpen) ? (<React.Fragment><div style={{display:'flex', alignItems:'center', gap:'6px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', padding:'6px 9px'}}><input type="date" value={dashCustomFrom} onChange={onDashCustomFrom} style={{background:'#0c0d10', border:'1px solid #23262d', borderRadius:'7px', color:'#e7e9ee', fontSize:'16px', padding:'5px 7px'}} /><span style={{color:'#5a5f68', fontSize:'12px'}}>–</span><input type="date" value={dashCustomTo} onChange={onDashCustomTo} style={{background:'#0c0d10', border:'1px solid #23262d', borderRadius:'7px', color:'#e7e9ee', fontSize:'16px', padding:'5px 7px'}} /></div></React.Fragment>) : null}</div><div style={{fontSize:'12.5px', color:'#6b7079', fontWeight:'500'}}>Periodo: <b style={{color:'#9297a1'}}>{dashPeriodLabel}</b></div></div>{/* KPI: sempre visibili, coerenti con il filtro sopra */}<div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:'14px', marginBottom:'24px'}}>{(kpis || []).map((k, kIdx) => (<React.Fragment key={kIdx}><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'16px 18px'}}><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.4px'}}>{k.label}</div><div style={k.valStyle}>{k.value}</div><div style={{fontSize:'11.5px', color:'#5a5f68', marginTop:'3px'}}>{k.sub}</div></div></React.Fragment>))}</div>{/* Scheda: Patrimonio netto */}{(isDashNetWorth) ? (<React.Fragment><div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(360px,1fr))', gap:'16px'}}><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'20px'}}><div style={{display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'14px', flexWrap:'wrap'}}><div><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.4px'}}>Patrimonio netto</div><div style={{fontFamily:'\'JetBrains Mono\',monospace', fontWeight:'700', fontSize:'24px', marginTop:'4px'}}>{nwTrend.currentText}</div></div><div style={{textAlign:'right'}}><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.4px'}}>Variazione</div><div style={nwTrend.changeStyle}>{nwTrend.changeText}</div></div></div><div style={{position:'relative', width:'100%', height:'200px', marginTop:'14px'}}><svg viewBox="0 0 100 60" preserveAspectRatio="none" style={{width:'100%', height:'100%', display:'block'}}><path d={nwTrend.areaPath} fill="rgba(91,141,239,0.14)" /><path d={nwTrend.linePath} fill="none" stroke="#5b8def" strokeWidth="2" vectorEffect="non-scaling-stroke" strokeLinejoin="round" strokeLinecap="round" /></svg>{(nwTrend.dots || []).map((d, dIdx) => (<React.Fragment key={dIdx}><div style={d.style} title={d.title}></div></React.Fragment>))}</div><div style={{display:'flex', justifyContent:'space-between', fontSize:'11px', color:'#5a5f68', marginTop:'8px'}}><span>{nwTrend.startLabel}</span><span>{nwTrend.endLabel}</span></div><p style={{margin:'16px 0 0', fontSize:'12px', color:'#5a5f68'}}>Somma dei saldi di tutti i conti (attività) al netto degli eventuali saldi negativi (passività), mese per mese.</p></div><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'20px'}}><h3 style={{margin:'0 0 14px', fontSize:'14px', fontWeight:'700'}}>Composizione attuale</h3>{(allocHasData) ? (<React.Fragment><div style={{padding:'36px', textAlign:'center', color:'#6b7079'}}>Nessun saldo positivo da mostrare.</div></React.Fragment>) : null}<div style={{display:'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'center' : 'center', gap:'20px'}}><div style={{position:'relative', width:'132px', height:'132px', flexShrink:'0'}}><svg viewBox="0 0 42 42" style={{width:'132px', height:'132px', transform:'rotate(-90deg)'}}><circle cx="21" cy="21" r="15.915" fill="none" stroke="#16181d" strokeWidth="5" />{(allocSegs || []).map((s, sIdx) => (<React.Fragment key={sIdx}><circle cx="21" cy="21" r="15.915" fill="none" stroke={s.color} strokeWidth="5" strokeDasharray={s.dash} strokeDashoffset={s.offset} /></React.Fragment>))}</svg><div style={{position:'absolute', inset:'0', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center'}}><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.4px'}}>Totale</div><div style={{fontFamily:'\'JetBrains Mono\',monospace', fontWeight:'600', fontSize:'13px'}}>{allocTotalText}</div></div></div><div style={{flex:'1', minWidth:'0', width: isMobile ? '100%' : 'auto', display:'flex', flexDirection:'column', gap:'9px'}}>{(allocSegs || []).map((s, sIdx) => (<React.Fragment key={sIdx}><div style={{display:'flex', alignItems:'center', gap:'9px'}}><span style={s.dotStyle}></span><span style={{flex:'1', fontSize:'12.5px', color:'#c8ccd2', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{s.label}</span><span style={{fontFamily:'\'JetBrains Mono\',monospace', fontSize:'12px', color:'#9297a1'}}>{s.valueText}</span><span style={{fontSize:'11px', color:'#5a5f68', width:'34px', textAlign:'right'}}>{s.pctText}</span></div></React.Fragment>))}</div></div></div></div></React.Fragment>) : null}{/* Scheda: Spese */}{(isDashSpending) ? (<React.Fragment><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'20px'}}><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:'14px', flexWrap:'wrap', marginBottom:'6px'}}><h3 style={{margin:'0', fontSize:'14px', fontWeight:'700'}}>Spese per categoria</h3><div style={{display:'flex', alignItems:'center', gap:'8px', flexWrap:'wrap'}}><select value={dashGroupId} onChange={onDashGroup} style={{padding:'6px 9px', background:'#131519', border:'1px solid #21242b', borderRadius:'8px', color:'#e7e9ee', fontSize:'16px', fontWeight:'500'}} style-focus="border-color:#5b8def;"><option value="">Tutti i gruppi</option>{(dashGroupOptions || []).map((opt, optIdx) => (<React.Fragment key={optIdx}><option value={opt.value}>{opt.label}</option></React.Fragment>))}</select><select value={dashCategoryId} onChange={onDashCategory} style={{padding:'6px 9px', background:'#131519', border:'1px solid #21242b', borderRadius:'8px', color:'#e7e9ee', fontSize:'16px', fontWeight:'500'}} style-focus="border-color:#5b8def;"><option value="">Tutte le categorie</option>{(dashCategoryOptions || []).map((opt, optIdx) => (<React.Fragment key={optIdx}><option value={opt.value}>{opt.label}</option></React.Fragment>))}</select>{(dashFilterActive) ? (<React.Fragment><button onClick={clearDashFilter} style={{display:'inline-flex', alignItems:'center', gap:'6px', padding:'6px 10px', background:'rgba(91,141,239,0.12)', border:'1px solid rgba(91,141,239,0.35)', borderRadius:'8px', color:'#8fb4f5', fontSize:'11.5px', fontWeight:'600', cursor:'pointer'}} {...hoverStyle({display:'inline-flex', alignItems:'center', gap:'6px', padding:'6px 10px', background:'rgba(91,141,239,0.12)', border:'1px solid rgba(91,141,239,0.35)', borderRadius:'8px', color:'#8fb4f5', fontSize:'11.5px', fontWeight:'600', cursor:'pointer'}, {background:'rgba(91,141,239,0.2)'})}>{dashFilterLabel} ×</button></React.Fragment>) : null}</div></div><div style={{marginBottom:'20px'}}><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.4px'}}>Totale speso</div><div style={{fontFamily:'\'JetBrains Mono\',monospace', fontWeight:'700', fontSize:'28px', marginTop:'4px'}}>{spendTotalText}</div></div>{(spendEmpty) ? (<React.Fragment><div style={{padding:'40px', textAlign:'center', color:'#6b7079'}}>Nessuna spesa nel periodo selezionato.</div></React.Fragment>) : null}<div style={{display:'flex', alignItems:'flex-start', gap:'26px', flexWrap:'wrap'}}><div style={{position:'relative', width:'150px', height:'150px', flexShrink:'0'}}><svg viewBox="0 0 42 42" style={{width:'150px', height:'150px', transform:'rotate(-90deg)'}}><circle cx="21" cy="21" r="15.915" fill="none" stroke="#16181d" strokeWidth="5" />{(spendSegs || []).map((s, sIdx) => (<React.Fragment key={sIdx}><circle cx="21" cy="21" r="15.915" fill="none" stroke={s.color} strokeWidth="5" strokeDasharray={s.dash} strokeDashoffset={s.offset} /></React.Fragment>))}</svg></div><div style={{flex:'1', minWidth:'230px', display:'flex', flexDirection:'column', gap:'2px'}}>{(spendCats || []).map((c, cIdx) => (<React.Fragment key={cIdx}><div onClick={c.onClick} style={{display:'grid', gridTemplateColumns:'16px 1fr 90px 44px', gap:'10px', alignItems:'center', padding:'9px 0', cursor:'pointer', borderBottom:'1px solid #16181d'}} {...hoverStyle({display:'grid', gridTemplateColumns:'16px 1fr 90px 44px', gap:'10px', alignItems:'center', padding:'9px 0', cursor:'pointer', borderBottom:'1px solid #16181d'}, {opacity:'0.82'})}><span style={c.dotStyle}></span><div style={{minWidth:'0'}}><div style={{fontSize:'13px', fontWeight:'600', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{c.name}</div><div style={{height:'4px', marginTop:'5px', background:'#1c1f26', borderRadius:'3px', overflow:'hidden'}}><div style={c.barStyle}></div></div></div><span style={{textAlign:'right', fontFamily:'\'JetBrains Mono\',monospace', fontSize:'12.5px'}}>{c.amountText}</span><span style={{textAlign:'right', fontSize:'11px', color:'#5a5f68'}}>{c.pctText}</span></div></React.Fragment>))}</div></div></div></React.Fragment>) : null}{/* Scheda: Entrate vs Uscite */}{(isDashIncomeExpense) ? (<React.Fragment><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'20px'}}><div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:'14px', marginBottom:'20px'}}>{(ieSummary || []).map((k, kIdx) => (<React.Fragment key={kIdx}><div><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.4px'}}>{k.label}</div><div style={k.valStyle}>{k.value}</div></div></React.Fragment>))}</div><div style={{display:'flex', alignItems:'center', gap:'14px', fontSize:'11.5px', color:'#9297a1', marginBottom:'14px'}}><span style={{display:'inline-flex', alignItems:'center', gap:'5px'}}><span style={{width:'9px', height:'9px', borderRadius:'2px', background:'#3ecf8e'}}></span>Entrate</span><span style={{display:'inline-flex', alignItems:'center', gap:'5px'}}><span style={{width:'9px', height:'9px', borderRadius:'2px', background:'#f0616d'}}></span>Uscite</span></div><div style={{display:'flex', alignItems:'flex-end', gap:'8px', overflowX:'auto', paddingBottom:'4px'}}>{(ieBars || []).map((b, bIdx) => (<React.Fragment key={bIdx}><div onClick={b.onClick} title={b.title} style={{flex:'1', minWidth:'40px', display:'flex', flexDirection:'column', alignItems:'center', gap:'7px', cursor:'pointer'}} {...hoverStyle({flex:'1', minWidth:'40px', display:'flex', flexDirection:'column', alignItems:'center', gap:'7px', cursor:'pointer'}, {opacity:'0.82'})}><div style={{width:'100%', height:'180px', display:'flex', alignItems:'flex-end', justifyContent:'center', gap:'3px'}}><div style={b.incomeStyle}></div><div style={b.expenseStyle}></div></div><div style={{fontSize:'11px', color:'#6b7079', whiteSpace:'nowrap'}}>{b.label}</div><div style={b.netStyle}>{b.netText}</div></div></React.Fragment>))}</div><div style={{fontSize:'11px', color:'#5a5f68', marginTop:'12px'}}>Clicca un mese per aprire i movimenti corrispondenti · il numero sotto ogni colonna è il risparmio netto del mese</div>{(ieInsight) ? (<React.Fragment><div style={{marginTop:'16px', paddingTop:'16px', borderTop:'1px solid #1c1f26', display:'flex', alignItems:'center', gap:'10px'}}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#8b7cf6" strokeWidth="1.8" style={{flexShrink:'0'}}><circle cx="12" cy="12" r="9" /><path d="M12 8v5" strokeLinecap="round" /><circle cx="12" cy="16" r="0.6" fill="#8b7cf6" /></svg><span style={{fontSize:'13px', fontWeight:'600', color:'#c8ccd2'}}>{ieInsight}</span></div></React.Fragment>) : null}</div></React.Fragment>) : null}{/* Scheda: Entrate per fonte */}{(isDashIncomeBreakdown) ? (<React.Fragment><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'20px'}}><div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:'16px'}}><h3 style={{margin:'0', fontSize:'14px', fontWeight:'700'}}>Entrate per fonte</h3><span style={{fontSize:'12.5px', color:'#9297a1'}}>Totale <b style={{fontFamily:'\'JetBrains Mono\',monospace', color:'#e7e9ee'}}>{incomeTotalText}</b></span></div>{(incomeBreakdownEmpty) ? (<React.Fragment><div style={{padding:'40px', textAlign:'center', color:'#6b7079'}}>Nessuna entrata nel periodo selezionato.</div></React.Fragment>) : null}<div style={{display:'flex', alignItems:'flex-start', gap:'26px', flexWrap:'wrap'}}><div style={{position:'relative', width:'150px', height:'150px', flexShrink:'0'}}><svg viewBox="0 0 42 42" style={{width:'150px', height:'150px', transform:'rotate(-90deg)'}}><circle cx="21" cy="21" r="15.915" fill="none" stroke="#16181d" strokeWidth="5" />{(incomeSegs || []).map((s, sIdx) => (<React.Fragment key={sIdx}><circle cx="21" cy="21" r="15.915" fill="none" stroke={s.color} strokeWidth="5" strokeDasharray={s.dash} strokeDashoffset={s.offset} /></React.Fragment>))}</svg></div><div style={{flex:'1', minWidth:'230px', display:'flex', flexDirection:'column', gap:'2px'}}>{(incomeRows || []).map((c, cIdx) => (<React.Fragment key={cIdx}><div onClick={c.onClick} style={{display:'grid', gridTemplateColumns:'16px 1fr 90px 44px', gap:'10px', alignItems:'center', padding:'9px 0', cursor:'pointer', borderBottom:'1px solid #16181d'}} {...hoverStyle({display:'grid', gridTemplateColumns:'16px 1fr 90px 44px', gap:'10px', alignItems:'center', padding:'9px 0', cursor:'pointer', borderBottom:'1px solid #16181d'}, {opacity:'0.82'})}><span style={c.dotStyle}></span><div style={{minWidth:'0'}}><div style={{fontSize:'13px', fontWeight:'600', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{c.name}</div><div style={{height:'4px', marginTop:'5px', background:'#1c1f26', borderRadius:'3px', overflow:'hidden'}}><div style={c.barStyle}></div></div></div><span style={{textAlign:'right', fontFamily:'\'JetBrains Mono\',monospace', fontSize:'12.5px'}}>{c.amountText}</span><span style={{textAlign:'right', fontSize:'11px', color:'#5a5f68'}}>{c.pctText}</span></div></React.Fragment>))}</div></div></div></React.Fragment>) : null}{/* Scheda: Risparmio & Età del denaro */}{(isDashSavings) ? (<React.Fragment><div style={{display:'flex', flexDirection:'column', gap:'16px'}}><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'20px'}}><div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))', gap:'14px'}}>{(savSummary || []).map((k, kIdx) => (<React.Fragment key={kIdx}><div><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.4px'}}>{k.label}</div><div style={k.valStyle}>{k.value}</div></div></React.Fragment>))}</div></div><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'20px'}}><h3 style={{margin:'0 0 14px', fontSize:'14px', fontWeight:'700'}}>Tasso di risparmio mensile</h3>{(savEmpty) ? (<React.Fragment><div style={{padding:'40px', textAlign:'center', color:'#6b7079'}}>Nessun dato nel periodo selezionato.</div></React.Fragment>) : null}<div style={{display:'flex', flexDirection:'column', gap:'2px'}}>{(savRows || []).map((r, rIdx) => (<React.Fragment key={rIdx}><div onClick={r.onClick} style={{display:'grid', gridTemplateColumns:'48px 1fr 54px 92px', gap:'12px', alignItems:'center', padding:'8px 0', cursor:'pointer', borderBottom:'1px solid #16181d'}} {...hoverStyle({display:'grid', gridTemplateColumns:'48px 1fr 54px 92px', gap:'12px', alignItems:'center', padding:'8px 0', cursor:'pointer', borderBottom:'1px solid #16181d'}, {opacity:'0.82'})}><span style={{fontSize:'11.5px', color:'#9297a1'}}>{r.label}</span><div style={{height:'8px', background:'#16181d', borderRadius:'4px', overflow:'hidden'}}><div style={r.barStyle}></div></div><span style={r.rateStyle}>{r.rateText}</span><span style={{textAlign:'right', fontFamily:'\'JetBrains Mono\',monospace', fontSize:'12px', color:'#9297a1'}}>{r.netText}</span></div></React.Fragment>))}</div><p style={{margin:'14px 0 0', fontSize:'11px', color:'#5a5f68'}}>Tasso di risparmio = (entrate − uscite) / entrate del mese. Clicca una riga per aprire i movimenti di quel mese.</p></div></div></React.Fragment>) : null}</div></React.Fragment>) : null}{/* ============ BUDGET ============ */}{(isBudget) ? (<React.Fragment><div style={{maxWidth:'940px', margin:'0 auto'}}><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:'16px', flexWrap:'wrap', marginBottom:'20px'}}><div style={{display:'flex', alignItems:'center', gap:'6px'}}><button onClick={onPrevMonth} style={{width:'34px', height:'34px', display:'flex', alignItems:'center', justifyContent:'center', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', cursor:'pointer'}} {...hoverStyle({width:'34px', height:'34px', display:'flex', alignItems:'center', justifyContent:'center', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', cursor:'pointer'}, {borderColor:'#2d313a'})}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="15 18 9 12 15 6" /></svg></button><div style={{minWidth:'150px', textAlign:'center', fontWeight:'600', fontSize:'15px'}}>{monthLabel}</div><button onClick={onNextMonth} style={{width:'34px', height:'34px', display:'flex', alignItems:'center', justifyContent:'center', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', cursor:'pointer'}} {...hoverStyle({width:'34px', height:'34px', display:'flex', alignItems:'center', justifyContent:'center', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', cursor:'pointer'}, {borderColor:'#2d313a'})}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6" /></svg></button></div><div style={readyCardStyle}><div style={{fontSize:'11px', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.5px', opacity:'0.85'}}>{readyLabel}</div><div style={{fontFamily:'\'JetBrains Mono\',monospace', fontWeight:'600', fontSize:'20px', marginTop:'1px'}}>{readyText}</div></div></div><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', overflow:'hidden'}}><div style={{display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(46px,64px) minmax(40px,54px) minmax(46px,64px)', gap:'8px', padding:'12px 18px', borderBottom:'1px solid #1c1f26', fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.4px'}}><div style={{overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>Categoria</div><div style={{textAlign:'right', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{isMobile ? 'Ass.' : 'Assegnato'}</div><div style={{textAlign:'right', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>Speso</div><div style={{textAlign:'right', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{isMobile ? 'Disp.' : 'Disponibile'}</div></div>{(budgetGroups || []).map((grp, grpIdx) => (<React.Fragment key={grpIdx}><div style={{display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(52px,90px)', gap:'8px', alignItems:'center', padding:'11px 18px', background:'#0e1013', borderBottom:'1px solid #1c1f26'}}><div style={{fontSize:'11px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.5px', color:'#8a8f98'}}>{grp.name}</div><div style={{textAlign:'right', fontFamily:'\'JetBrains Mono\',monospace', fontSize:'11.5px', color:'#6b7079'}}>{grp.assignedText}</div></div>{(grp.rows || []).map((row, rowIdx) => (<React.Fragment key={rowIdx}><div style={{display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(46px,64px) minmax(40px,54px) minmax(46px,64px)', gap:'8px', alignItems:'center', padding:'13px 18px', borderBottom:'1px solid #16181d'}}><div style={{minWidth:'0'}}><div style={{fontWeight:'600', fontSize:'13.5px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{row.name}</div>{(row.hasHint) ? (<React.Fragment><div style={row.hintStyle}>{row.hint}</div></React.Fragment>) : null}<div style={{height:'5px', marginTop:'7px', background:'#1c1f26', borderRadius:'4px', overflow:'hidden'}}><div style={row.barStyle}></div></div></div><div style={{display:'flex', justifyContent:'flex-end'}}><input type="text" inputMode="decimal" value={row.assignedValue} onChange={row.onAssign} style={{width:'100%', minWidth:'0', textAlign:'right', padding:'7px 6px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'8px', color:'#e7e9ee', fontFamily:'\'JetBrains Mono\',monospace', fontSize:'16px'}} style-focus="border-color:#5b8def;" /></div><div style={{textAlign:'right', fontFamily:'\'JetBrains Mono\',monospace', fontSize:'13px', color:'#9297a1'}}>{row.spentText}</div><div style={{textAlign:'right'}}><span style={row.availStyle}>{row.availText}</span></div></div></React.Fragment>))}</React.Fragment>))}{(budgetEmpty) ? (<React.Fragment><div style={{padding:'44px 18px', textAlign:'center', color:'#6b7079'}}>Nessuna categoria di uscita. Creane una nella sezione Categorie.</div></React.Fragment>) : null}</div><p style={{margin:'14px 2px 0', fontSize:'12px', color:'#5a5f68'}}>Lo <b style={{color:'#8a8f98'}}>speso</b> è calcolato dai movimenti del mese; il <b style={{color:'#8a8f98'}}>disponibile</b> è assegnato − speso.</p></div></React.Fragment>) : null}{/* ============ MOVIMENTI ============ */}{(isTransactions) ? (<React.Fragment><div style={{maxWidth:'1080px', margin:'0 auto'}}><div style={{display:'flex', justifyContent:'flex-end', gap:'8px', marginBottom:'14px'}}><button onClick={onExportCsv} style={{display:'inline-flex', alignItems:'center', gap:'7px', padding:'8px 13px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', fontSize:'12.5px', fontWeight:'600', cursor:'pointer'}} {...hoverStyle({display:'inline-flex', alignItems:'center', gap:'7px', padding:'8px 13px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', fontSize:'12.5px', fontWeight:'600', cursor:'pointer'}, {borderColor:'#2d313a'})}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3v12" /><polyline points="7 11 12 16 17 11" /><path d="M5 20h14" /></svg>
            Esporta CSV
            ...
            Importa CSV
          </button></div><div style={{display:'flex', alignItems:'flex-end', gap:'12px', flexWrap:'wrap', marginBottom:'18px'}}><label style={{display:'flex', flexDirection:'column', gap:'5px', fontSize:'11px', color:'#6b7079', fontWeight:'600'}}>Dal
            <input type="date" value={filters.from} onChange={onFilter} data-filter="from" style={{padding:'8px 10px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', fontSize:'16px', fontWeight:'500'}} style-focus="border-color:#5b8def;" /></label><label style={{display:'flex', flexDirection:'column', gap:'5px', fontSize:'11px', color:'#6b7079', fontWeight:'600'}}>Al
            <input type="date" value={filters.to} onChange={onFilter} data-filter="to" style={{padding:'8px 10px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', fontSize:'16px', fontWeight:'500'}} style-focus="border-color:#5b8def;" /></label><label style={{display:'flex', flexDirection:'column', gap:'5px', fontSize:'11px', color:'#6b7079', fontWeight:'600'}}>Conto
            <select value={filters.accountId} onChange={onFilter} data-filter="accountId" style={{padding:'8px 10px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', fontSize:'16px', fontWeight:'500', minWidth:'150px'}} style-focus="border-color:#5b8def;"><option value="">Tutti</option>{(accountFilterOptions || []).map((opt, optIdx) => (<React.Fragment key={optIdx}><option value={opt.value}>{opt.label}</option></React.Fragment>))}</select></label><label style={{display:'flex', flexDirection:'column', gap:'5px', fontSize:'11px', color:'#6b7079', fontWeight:'600'}}>Categoria
            <select value={filters.categoryId} onChange={onFilter} data-filter="categoryId" style={{padding:'8px 10px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', fontSize:'16px', fontWeight:'500', minWidth:'150px'}} style-focus="border-color:#5b8def;"><option value="">Tutte</option>{(categoryFilterOptions || []).map((opt, optIdx) => (<React.Fragment key={optIdx}><option value={opt.value}>{opt.label}</option></React.Fragment>))}</select></label><label style={{display:'flex', flexDirection:'column', gap:'5px', fontSize:'11px', color:'#6b7079', fontWeight:'600'}}>Tipo
            <select value={filters.type} onChange={onFilter} data-filter="type" style={{padding:'8px 10px', background:'#131519', border:'1px solid #21242b', borderRadius:'9px', color:'#e7e9ee', fontSize:'16px', fontWeight:'500', minWidth:'130px'}} style-focus="border-color:#5b8def;"><option value="">Tutti</option><option value="Income">Entrata</option><option value="Expense">Uscita</option><option value="Transfer">Trasferimento</option></select></label>{(hasFilters) ? (<React.Fragment><button onClick={clearFilters} style={{padding:'8px 12px', background:'transparent', border:'1px solid #21242b', borderRadius:'9px', color:'#9297a1', fontSize:'12.5px', fontWeight:'500', cursor:'pointer', height:'35px'}} {...hoverStyle({padding:'8px 12px', background:'transparent', border:'1px solid #21242b', borderRadius:'9px', color:'#9297a1', fontSize:'12.5px', fontWeight:'500', cursor:'pointer', height:'35px'}, {borderColor:'#2d313a', color:'#e7e9ee'})}>Azzera filtri</button></React.Fragment>) : null}</div><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', overflow:'hidden'}}>{(txnRows || []).map((t, tIdx) => (<React.Fragment key={tIdx}><div style={{padding:'12px 16px', borderBottom:'1px solid #16181d', display:'flex', flexDirection:'column', gap:'7px'}} {...hoverStyle({padding:'12px 16px', borderBottom:'1px solid #16181d', display:'flex', flexDirection:'column', gap:'7px'}, {background:'#131519'})}><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:'10px'}}><span style={{fontFamily:'\'JetBrains Mono\',monospace', fontSize:'11px', color:'#5a5f68'}}>{t.dateLabel}</span><div style={{display:'flex', gap:'2px'}}><button onClick={t.onEdit} title="Modifica" style={{width:'25px', height:'25px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'7px', color:'#6b7079', cursor:'pointer', flexShrink:'0'}} {...hoverStyle({width:'25px', height:'25px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'7px', color:'#6b7079', cursor:'pointer', flexShrink:'0'}, {borderColor:'#2d313a', color:'#e7e9ee'})}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z" /></svg></button><button onClick={t.onDelete} title="Elimina" style={{width:'25px', height:'25px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'7px', color:'#6b7079', cursor:'pointer', flexShrink:'0'}} {...hoverStyle({width:'25px', height:'25px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'7px', color:'#6b7079', cursor:'pointer', flexShrink:'0'}, {borderColor:'#3a2426', color:'#f0616d'})}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 21 6" /><path d="M8 6V4h8v2" /><path d="M6 6l1 14h10l1-14" /></svg></button></div></div><div style={{display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'12px'}}><div style={{minWidth:'0', flex:'1'}}><div style={{fontWeight:'700', fontSize:'14.5px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{t.description}</div>{(t.hasSub) ? (<React.Fragment><div style={{fontSize:'11.5px', color:'#5a5f68', marginTop:'1px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{t.subLabel}</div></React.Fragment>) : null}</div><div style={{display:'flex', alignItems:'center', gap:'6px', flexShrink:'0'}}><span title={t.clearedTitle} style={t.clearedStyle}></span><span style={t.amountStyle}>{t.amountText}</span></div></div><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:'10px'}}><span style={t.tagStyle}>{t.categoryName}</span><span style={{fontSize:'11.5px', color:'#6b7079', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', textAlign:'right'}}>{t.accountName}</span></div></div></React.Fragment>))}{(txnEmpty) ? (<React.Fragment><div style={{padding:'52px 18px', textAlign:'center', color:'#6b7079'}}>Nessun movimento{emptyFilterSuffix}.</div></React.Fragment>) : null}</div></div></React.Fragment>) : null}{/* ============ CONTI ============ */}{(isAccounts) ? (<React.Fragment><div style={{maxWidth:'1000px', margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'16px'}}>{(accountCards || []).map((a, aIdx) => (<React.Fragment key={aIdx}><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'18px'}} {...hoverStyle({background:'#101216', border:'1px solid #1c1f26', borderRadius:'14px', padding:'18px'}, {borderColor:'#2a2e37'})}><div style={{display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'10px'}}><div style={{minWidth:'0'}}><div style={{fontWeight:'700', fontSize:'15px', letterSpacing:'-0.2px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{a.name}</div><div style={{fontSize:'12.5px', color:'#6b7079', marginTop:'2px'}}>{a.bank}</div></div><div style={{display:'flex', gap:'4px', flexShrink:'0'}}><button onClick={a.onEdit} title="Modifica" style={{width:'28px', height:'28px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'7px', color:'#6b7079', cursor:'pointer'}} {...hoverStyle({width:'28px', height:'28px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'7px', color:'#6b7079', cursor:'pointer'}, {borderColor:'#2d313a', color:'#e7e9ee'})}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z" /></svg></button><button onClick={a.onDelete} title="Elimina" style={{width:'28px', height:'28px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'7px', color:'#6b7079', cursor:'pointer'}} {...hoverStyle({width:'28px', height:'28px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'7px', color:'#6b7079', cursor:'pointer'}, {borderColor:'#3a2426', color:'#f0616d'})}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 21 6" /><path d="M8 6V4h8v2" /><path d="M6 6l1 14h10l1-14" /></svg></button></div></div><div style={{marginTop:'16px'}}><div style={{fontSize:'11px', color:'#6b7079', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.4px'}}>Saldo attuale</div><div style={a.balanceStyle}>{a.balanceText}</div></div><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:'14px', paddingTop:'14px', borderTop:'1px solid #1c1f26'}}><div style={{display:'flex', gap:'6px', alignItems:'center', minWidth:'0'}}><span style={a.typeBadgeStyle}>{a.typeLabel}</span><span style={a.budgetBadgeStyle}>{a.budgetBadgeLabel}</span></div><span style={{fontSize:'11.5px', color:'#5a5f68', whiteSpace:'nowrap'}}>iniz. {a.initialText}</span></div></div></React.Fragment>))}{(accountsEmpty) ? (<React.Fragment><div style={{gridColumn:'1/-1', padding:'52px', textAlign:'center', color:'#6b7079', background:'#101216', border:'1px dashed #23262d', borderRadius:'14px'}}>Nessun conto. Aggiungine uno per iniziare.</div></React.Fragment>) : null}</div></React.Fragment>) : null}{/* ============ CATEGORIE ============ */}{(isCategories) ? (<React.Fragment><div style={{maxWidth:'900px', margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))', gap:'20px'}}><div><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'12px'}}><h3 style={{margin:'0', fontSize:'13px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.5px', color:'#3ecf8e'}}>Entrate</h3><button onClick={onNewIncomeCat} style={{fontSize:'12px', color:'#5b8def', background:'none', border:'none', cursor:'pointer', fontWeight:'600'}}>+ Aggiungi</button></div><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'12px', overflow:'hidden'}}>{(incomeCats || []).map((c, cIdx) => (<React.Fragment key={cIdx}><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'13px 16px', borderBottom:'1px solid #16181d'}} {...hoverStyle({display:'flex', alignItems:'center', justifyContent:'space-between', padding:'13px 16px', borderBottom:'1px solid #16181d'}, {background:'#131519'})}><span style={{fontWeight:'600', fontSize:'13.5px'}}>{c.name}</span><div style={{display:'flex', gap:'4px'}}><button onClick={c.onEdit} style={{width:'26px', height:'26px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}} {...hoverStyle({width:'26px', height:'26px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}, {borderColor:'#2d313a', color:'#e7e9ee'})}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z" /></svg></button><button onClick={c.onDelete} style={{width:'26px', height:'26px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}} {...hoverStyle({width:'26px', height:'26px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}, {borderColor:'#3a2426', color:'#f0616d'})}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 21 6" /><path d="M8 6V4h8v2" /><path d="M6 6l1 14h10l1-14" /></svg></button></div></div></React.Fragment>))}{(incomeEmpty) ? (<React.Fragment><div style={{padding:'24px', textAlign:'center', color:'#6b7079', fontSize:'13px'}}>Nessuna categoria</div></React.Fragment>) : null}</div></div><div><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'12px'}}><h3 style={{margin:'0', fontSize:'13px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.5px', color:'#f0616d'}}>Uscite</h3><div style={{display:'flex', gap:'14px'}}><button onClick={onNewGroup} style={{fontSize:'12px', color:'#5b8def', background:'none', border:'none', cursor:'pointer', fontWeight:'600'}}>+ Gruppo</button><button onClick={onNewExpenseCat} style={{fontSize:'12px', color:'#5b8def', background:'none', border:'none', cursor:'pointer', fontWeight:'600'}}>+ Categoria</button></div></div><div style={{display:'flex', flexDirection:'column', gap:'16px'}}>{(expenseGroups || []).map((grp, grpIdx) => (<React.Fragment key={grpIdx}><div><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'7px', paddingLeft:'2px', minHeight:'22px'}}><span style={{fontSize:'11px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.5px', color:'#8a8f98'}}>{grp.name}</span>{(grp.isReal) ? (<React.Fragment><div style={{display:'flex', gap:'4px'}}><button onClick={grp.onEditGroup} title="Rinomina gruppo" style={{width:'24px', height:'24px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}} {...hoverStyle({width:'24px', height:'24px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}, {borderColor:'#2d313a', color:'#e7e9ee'})}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z" /></svg></button><button onClick={grp.onDeleteGroup} title="Elimina gruppo" style={{width:'24px', height:'24px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}} {...hoverStyle({width:'24px', height:'24px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}, {borderColor:'#3a2426', color:'#f0616d'})}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 21 6" /><path d="M8 6V4h8v2" /><path d="M6 6l1 14h10l1-14" /></svg></button></div></React.Fragment>) : null}</div><div style={{background:'#101216', border:'1px solid #1c1f26', borderRadius:'12px', overflow:'hidden'}}>{(grp.cats || []).map((c, cIdx) => (<React.Fragment key={cIdx}><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', borderBottom:'1px solid #16181d'}} {...hoverStyle({display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', borderBottom:'1px solid #16181d'}, {background:'#131519'})}><div style={{minWidth:'0'}}><div style={{fontWeight:'600', fontSize:'13.5px'}}>{c.name}</div>{(c.hasTarget) ? (<React.Fragment><div style={{fontSize:'11px', color:'#5a5f68', marginTop:'2px'}}>{c.targetText}</div></React.Fragment>) : null}</div><div style={{display:'flex', gap:'4px'}}><button onClick={c.onEdit} style={{width:'26px', height:'26px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}} {...hoverStyle({width:'26px', height:'26px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}, {borderColor:'#2d313a', color:'#e7e9ee'})}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z" /></svg></button><button onClick={c.onDelete} style={{width:'26px', height:'26px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}} {...hoverStyle({width:'26px', height:'26px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'1px solid transparent', borderRadius:'6px', color:'#6b7079', cursor:'pointer'}, {borderColor:'#3a2426', color:'#f0616d'})}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 21 6" /><path d="M8 6V4h8v2" /><path d="M6 6l1 14h10l1-14" /></svg></button></div></div></React.Fragment>))}{(grp.isEmptyReal) ? (<React.Fragment><div style={{padding:'14px 16px', fontSize:'12px', color:'#5a5f68'}}>Gruppo vuoto — assegna categorie modificandole.</div></React.Fragment>) : null}</div></div></React.Fragment>))}{(expenseEmpty) ? (<React.Fragment><div style={{padding:'24px', textAlign:'center', color:'#6b7079', fontSize:'13px', background:'#101216', border:'1px solid #1c1f26', borderRadius:'12px'}}>Nessuna categoria</div></React.Fragment>) : null}</div></div></div></React.Fragment>) : null}</div></main>{/* ============ MODALE ============ */}{(modalOpen) ? (<React.Fragment><div style={{position:'fixed', inset:'0', zIndex:'60', display:'flex', alignItems:'center', justifyContent:'center', padding:'20px', background:'rgba(0,0,0,0.6)', animation:'fpOverlay .18s ease'}} onClick={onBackdrop}><div style={{width:'100%', maxWidth:'460px', maxHeight:'90vh', overflowY:'auto', background:'#14161b', border:'1px solid #262a32', borderRadius:'16px', boxShadow:'0 24px 60px rgba(0,0,0,0.55)', animation:'fpFade .22s cubic-bezier(.2,.8,.3,1)'}} data-modal-card="1"><div style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 20px', borderBottom:'1px solid #21242b'}}><h2 style={{margin:'0', fontSize:'16px', fontWeight:'700'}}>{modalTitle}</h2><button onClick={onClose} style={{width:'30px', height:'30px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'none', borderRadius:'8px', color:'#6b7079', cursor:'pointer', fontSize:'20px'}} {...hoverStyle({width:'30px', height:'30px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'none', borderRadius:'8px', color:'#6b7079', cursor:'pointer', fontSize:'20px'}, {color:'#e7e9ee'})}>×</button></div><div style={{padding:'20px', display:'flex', flexDirection:'column', gap:'15px'}}>{/* ACCOUNT FORM */}{(isAccountModal) ? (<React.Fragment><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Nome
              <input type="text" value={data.name} onChange={onField} data-field="name" placeholder="es. Conto Corrente" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;" /></label><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Banca / Istituto
              <input type="text" value={data.bank} onChange={onField} data-field="bank" placeholder="es. Intesa Sanpaolo" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;" /></label><div style={{display:'flex', gap:'12px'}}><label style={{flex:'1', display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Tipo
                <select value={data.type} onChange={onField} data-field="type" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;">{(accountTypeOptions || []).map((opt, optIdx) => (<React.Fragment key={optIdx}><option value={opt.value}>{opt.label}</option></React.Fragment>))}</select></label><label style={{flex:'1', display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Saldo iniziale (€)
                <input type="text" inputMode="decimal" value={data.initialBalance} onChange={onField} data-field="initialBalance" placeholder="0,00" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px', fontFamily:'\'JetBrains Mono\',monospace'}} style-focus="border-color:#5b8def;" /></label></div><label style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:'12px', padding:'11px 13px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'10px', cursor:'pointer'}}><span style={{fontSize:'13px', color:'#e7e9ee', fontWeight:'600'}}>Conto nel budget<span style={{display:'block', fontSize:'11px', color:'#6b7079', fontWeight:'500', marginTop:'2px'}}>Disattiva per conti di monitoraggio (investimenti, pensione)</span></span><input type="checkbox" checked={data.onBudget} onChange={onField} data-field="onBudget" style={{fontSize:'16px', width:'18px', height:'18px', accentColor:'#3ecf8e', cursor:'pointer', flexShrink:'0'}} /></label></React.Fragment>) : null}{/* TRANSACTION FORM */}{(isTransactionModal) ? (<React.Fragment><div style={{display:'flex', gap:'8px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'10px', padding:'4px'}}><button onClick={setTypeIncome} style={typeBtn.income}>Entrata</button><button onClick={setTypeExpense} style={typeBtn.expense}>Uscita</button><button onClick={setTypeTransfer} style={typeBtn.transfer}>Trasferimento</button></div><div style={{display:'flex', gap:'12px'}}><label style={{flex:'1', display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Data
                <input type="date" value={data.date} onChange={onField} data-field="date" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;" /></label><label style={{flex:'1', display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Importo (€)
                <input type="text" inputMode="decimal" value={data.amount} onChange={onField} data-field="amount" placeholder="0,00" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px', fontFamily:'\'JetBrains Mono\',monospace'}} style-focus="border-color:#5b8def;" /></label></div><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Descrizione
              <input type="text" value={data.description} onChange={onField} data-field="description" placeholder="es. Spesa Esselunga" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;" /></label><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>{accountFieldLabel}
              <select value={data.accountId} onChange={onField} data-field="accountId" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;">{(modalAccountOptions || []).map((opt, optIdx) => (<React.Fragment key={optIdx}><option value={opt.value}>{opt.label}</option></React.Fragment>))}</select></label>{(isTransferModal) ? (<React.Fragment><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Conto di destinazione
                <select value={data.toAccountId} onChange={onField} data-field="toAccountId" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;">{(modalAccountOptions || []).map((opt, optIdx) => (<React.Fragment key={optIdx}><option value={opt.value}>{opt.label}</option></React.Fragment>))}</select></label></React.Fragment>) : null}{(showCategoryField) ? (<React.Fragment><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Categoria
                <select value={data.categoryId} onChange={onField} data-field="categoryId" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;"><option value="">— Nessuna —</option>{(modalCategoryOptions || []).map((opt, optIdx) => (<React.Fragment key={optIdx}><option value={opt.value}>{opt.label}</option></React.Fragment>))}</select></label></React.Fragment>) : null}<label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Note
              <input type="text" value={data.notes} onChange={onField} data-field="notes" placeholder="Facoltative" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;" /></label><div><div style={{fontSize:'12px', color:'#9297a1', fontWeight:'600', marginBottom:'6px'}}>Stato liquidazione</div><div style={{display:'flex', gap:'8px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'10px', padding:'4px'}}><button onClick={setClearedUncleared} style={clearedBtn.uncleared}>Non liquidato</button><button onClick={setClearedCleared} style={clearedBtn.cleared}>Liquidato</button><button onClick={setClearedReconciled} style={clearedBtn.reconciled}>Riconciliato</button></div></div></React.Fragment>) : null}{/* CATEGORY FORM */}{(isCategoryModal) ? (<React.Fragment><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Nome
              <input type="text" value={data.name} onChange={onField} data-field="name" placeholder="es. Spesa alimentare" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;" /></label><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Tipo
              <select value={data.type} onChange={onField} data-field="type" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;"><option value="Expense">Uscita</option><option value="Income">Entrata</option></select></label><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Gruppo
              <input type="text" list="fp-groups" value={data.group} onChange={onField} data-field="group" placeholder="es. Obblighi fissi" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;" /><datalist id="fp-groups">{(existingGroups || []).map((g, gIdx) => (<React.Fragment key={gIdx}><option value={g}></option></React.Fragment>))}</datalist></label>{(isExpenseCatModal) ? (<React.Fragment><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Obiettivo mensile (€)
                <input type="text" inputMode="decimal" value={data.target} onChange={onField} data-field="target" placeholder="Facoltativo—quanto vuoi assegnare ogni mese" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px', fontFamily:'\'JetBrains Mono\',monospace'}} style-focus="border-color:#5b8def;" /></label></React.Fragment>) : null}</React.Fragment>) : null}{/* IMPORT FORM */}{(isImportModal) ? (<React.Fragment><div style={{fontSize:'12.5px', color:'#9297a1', lineHeight:'1.6'}}>Carica un file <b style={{color:'#e7e9ee'}}>CSV</b> (separatore <span style={{color:'#5b8def', fontFamily:'\'JetBrains Mono\',monospace'}}>;</span>) oppure incolla i dati qui sotto. Il conto deve corrispondere per nome; le categorie mancanti vengono create.</div><div style={{background:'#0c0d10', border:'1px solid #21242b', borderRadius:'9px', padding:'10px 12px', fontFamily:'\'JetBrains Mono\',monospace', fontSize:'11px', color:'#6b7079', overflowX:'auto', whiteSpace:'nowrap'}}>Data;Descrizione;Conto;Conto destinazione;Tipo;Categoria;Importo;Note;Stato</div><div style={{display:'flex', gap:'12px', alignItems:'center', flexWrap:'wrap'}}><label style={{display:'inline-flex', alignItems:'center', gap:'7px', padding:'9px 14px', background:'#1c1f26', border:'1px solid #262a32', borderRadius:'9px', color:'#e7e9ee', fontSize:'13px', fontWeight:'600', cursor:'pointer'}} {...hoverStyle({display:'inline-flex', alignItems:'center', gap:'7px', padding:'9px 14px', background:'#1c1f26', border:'1px solid #262a32', borderRadius:'9px', color:'#e7e9ee', fontSize:'13px', fontWeight:'600', cursor:'pointer'}, {background:'#23262d'})}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 16V4" /><polyline points="7 9 12 4 17 9" /><path d="M5 20h14" /></svg>
                Scegli file
                <input type="file" accept=".csv,text/csv" onChange={onImportFile} style={{fontSize:'16px', display:'none'}} /></label><button onClick={onDownloadTemplate} style={{background:'none', border:'none', color:'#5b8def', fontSize:'12.5px', fontWeight:'600', cursor:'pointer', padding:'0'}}>Scarica modello</button>{(importHasFile) ? (<React.Fragment><span style={{fontSize:'12px', color:'#3ecf8e', fontWeight:'600'}}>{importFileName}</span></React.Fragment>) : null}</div><textarea value={data.text} onChange={onField} data-field="text" placeholder="01/07/2026;Spesa;Revolut;;Uscita;Spesa alimentare;42,50;;Non liquidato" rows="5" style={{width:'100%', resize:'vertical', padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px', color:'#e7e9ee', fontFamily:'\'JetBrains Mono\',monospace', lineHeight:'1.5'}} style-focus="border-color:#5b8def;"></textarea></React.Fragment>) : null}{/* GROUP FORM */}{(isGroupModal) ? (<React.Fragment><label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600'}}>Nome del gruppo
              <input type="text" value={data.name} onChange={onField} data-field="name" placeholder="es. Obblighi fissi" style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', fontSize:'16px'}} style-focus="border-color:#5b8def;" /></label><div style={{fontSize:'12px', color:'#6b7079', lineHeight:'1.5'}}>I gruppi organizzano le categorie di uscita nel budget. Rinominando un gruppo, tutte le categorie collegate vengono aggiornate.</div></React.Fragment>) : null}</div><div style={{display:'flex', gap:'10px', justifyContent:'space-between', alignItems:'center', padding:'16px 20px', borderTop:'1px solid #21242b'}}>{(canDelete) ? (<React.Fragment><button onClick={onModalDelete} style={{padding:'9px 14px', background:'transparent', border:'1px solid #3a2426', borderRadius:'9px', color:'#f0616d', fontSize:'13px', fontWeight:'600', cursor:'pointer'}} {...hoverStyle({padding:'9px 14px', background:'transparent', border:'1px solid #3a2426', borderRadius:'9px', color:'#f0616d', fontSize:'13px', fontWeight:'600', cursor:'pointer'}, {background:'#f0616d', color:'#fff', borderColor:'#f0616d'})}>Elimina</button></React.Fragment>) : null}<div style={{display:'flex', gap:'10px', marginLeft:'auto'}}><button onClick={onClose} style={{padding:'9px 16px', background:'#1c1f26', border:'1px solid #262a32', borderRadius:'9px', color:'#e7e9ee', fontSize:'13px', fontWeight:'600', cursor:'pointer'}} {...hoverStyle({padding:'9px 16px', background:'#1c1f26', border:'1px solid #262a32', borderRadius:'9px', color:'#e7e9ee', fontSize:'13px', fontWeight:'600', cursor:'pointer'}, {background:'#23262d'})}>Annulla</button><button onClick={onSave} style={{padding:'9px 18px', background:'#5b8def', border:'none', borderRadius:'9px', color:'#fff', fontSize:'13px', fontWeight:'600', cursor:'pointer'}} {...hoverStyle({padding:'9px 18px', background:'#5b8def', border:'none', borderRadius:'9px', color:'#fff', fontSize:'13px', fontWeight:'600', cursor:'pointer'}, {background:'#6f9bf2'})}>{saveLabel}</button></div></div></div></div></React.Fragment>) : null}</div>
    );
  }
}
