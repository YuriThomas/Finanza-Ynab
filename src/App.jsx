...
class App extends React.Component {
  ...
  buildViewModel(){
    ...
    // ---- Scheda: Macro categorie (= i tuoi gruppi di categorie di uscita) ----
    // Unificato coi gruppi già esistenti: ogni gruppo È una macro categoria, niente
    // da creare o assegnare a parte. Aggreghiamo, per il mese corrente, quanto
    // assegnato, speso e "necessario" (somma degli obiettivi) per ciascun gruppo.
    const macroBuckets = {};
    st.groups.forEach(g=>{ macroBuckets[g.name] = { id:g.name, name:g.name, assigned:0, spent:0, needed:0 }; });
    macroBuckets[''] = { id:'', name:'Senza gruppo', assigned:0, spent:0, needed:0 };
    expenseCatList.forEach(c=>{
      const key = (c.group && macroBuckets[c.group]) ? c.group : '';
      const bucket = macroBuckets[key];
