...
class App extends React.Component {
  ...
  seed(){
    ...
    return { Accounts:A, Transactions:T, Categories:C, Groups:G, Budgets:B, Settings:S, CategoryNotes:[], MacroCategories:[] };
  }

  refresh(){
    const s = this.store;
    this.setState({ accounts:s.all('Accounts'), transactions:s.all('Transactions'), categories:s.all('Categories'), groups:s.all('Groups'), budgets:s.all('Budgets'), categoryNotes:s.all('CategoryNotes'), macroCategories:s.all('MacroCategories') });
  }
  ...
  newGroup(){ this.openModal('group','create',{ name:'', fixed:false, macroCategoryId:'' }); }
  editGroup(name){ const g=this.state.groups.find(x=>x.name===name); this.openModal('group','edit',{ id:g?g.id:'', name:name, origName:name, fixed: g?!!g.fixed:false, macroCategoryId: g?(g.macroCategoryId||''):'' }); }
  newMacroCategory(){ this.openModal('macro','create',{ name:'' }); }
  editMacroCategory(mc){ this.openModal('macro','edit',{ id:mc.id, name:mc.name }); }
  delMacroCategory(mc){
    const groupsUsingIt = this.state.groups.filter(g=>g.macroCategoryId===mc.id);
    if(!this.confirmDel('Eliminare la macro categoria "'+mc.name+'"?'+(groupsUsingIt.length?(' I '+groupsUsingIt.length+' gruppi collegati resteranno senza macro categoria.'):''))) return;
    groupsUsingIt.forEach(g=>this.store.update('Groups', g.id, { macroCategoryId:'' }));
    this.store.remove('MacroCategories', mc.id);
    this.refresh();
  }
  delGroup(name){
  ...
  saveModal(){
    ...
    if (m.kind==='group'){
      ...
      if(m.mode==='edit'){
        if(d.id) this.store.update('Groups', d.id, { name, fixed: !!d.fixed, macroCategoryId: d.macroCategoryId||'' });
        ...
        this.store.create('Groups', { name, fixed: !!d.fixed, macroCategoryId: d.macroCategoryId||'' });
      }
      this.closeModal(); this.refresh(); return;
    }
    if (m.kind==='macro'){
      const name=(d.name||'').trim(); if(!name){ this.closeModal(); return; }
      if(m.mode==='edit' && d.id) this.store.update('MacroCategories', d.id, { name });
      else if(!this.state.macroCategories.some(mc=>mc.name===name)) this.store.create('MacroCategories', { name });
      this.closeModal(); this.refresh(); return;
    }
    if (m.kind==='account'){
  ...
  deleteFromModal(){ const m=this.state.modal; const d=m.data; const map={account:'Accounts',transaction:'Transactions',category:'Categories'}; if(m.kind==='group'){ this.closeModal(); this.delGroup(d.origName); return; } if(m.kind==='macro'){ const mc=this.state.macroCategories.find(x=>x.id===d.id); this.closeModal(); if(mc) this.delMacroCategory(mc); return; } if(this.confirmDel('Eliminare definitivamente?')){ this.store.remove(map[m.kind], d.id); this.closeModal(); this.refresh(); } }
  ...
  buildViewModel(){
    ...
    const dashTabDefs = [
      ...
      ['spending','Spese per categoria'], ['macro','Macro categorie'], ['goals','Obiettivi'],
      ['struggling','Categorie in affanno'],
    ...
    // ---- Scheda: Macro categorie (raggruppamenti liberi definiti dall'utente) ----
    // Ogni gruppo di categorie può essere assegnato a una macro categoria (da Categorie
    // → modifica gruppo). Qui aggreghiamo, per il mese corrente, quanto assegnato,
    // speso e "necessario" (somma degli obiettivi delle categorie) per ciascuna.
    const macroGroupMap = {}; st.groups.forEach(g=>{ macroGroupMap[g.name] = g.macroCategoryId||''; });
    const macroBuckets = {};
    st.macroCategories.forEach(mc=>{ macroBuckets[mc.id] = { id:mc.id, name:mc.name, assigned:0, spent:0, needed:0 }; });
    macroBuckets[''] = { id:'', name:'Senza macro categoria', assigned:0, spent:0, needed:0 };
    expenseCatList.forEach(c=>{
      const macroId = macroGroupMap[c.group||''] || '';
      const bucket = macroBuckets[macroId] || macroBuckets[''];
      bucket.assigned += this.catAssignedMonth(c.id, goalsMonth);
      bucket.spent += this.catSpentMonth(c.id, goalsMonth);
      bucket.needed += this.catTargetNeeded(c, goalsMonth);
    });
    const macroRowsRaw = Object.values(macroBuckets).filter(b=>b.assigned>0.005 || b.spent>0.005 || b.needed>0.005).sort((a,b)=> (b.assigned+b.spent) - (a.assigned+a.spent));
    const macroMax = Math.max(1, ...macroRowsRaw.map(b=>Math.max(b.assigned,b.spent,b.needed)));
    const macroRows = macroRowsRaw.map(b => {
      const fundedPct = b.needed>0.005 ? Math.min(100,(b.assigned/b.needed)*100) : null;
      return {
        id:b.id, name:b.name,
        assignedText:this.fmtEur(b.assigned), spentText:this.fmtEur(b.spent),
        neededText: b.needed>0.005 ? this.fmtEur(b.needed) : '',
        hasNeeded: b.needed>0.005,
        fundedPctText: fundedPct==null ? '' : Math.round(fundedPct)+'%',
        plannedBarStyle:{ width: Math.max(2,(b.assigned/macroMax)*100)+'%', height:'100%', background:'#5b8def', borderRadius:'4px' },
        actualBarStyle:{ width: Math.max(2,(b.spent/macroMax)*100)+'%', height:'100%', background: b.spent>b.assigned+0.005 ? '#f0616d' : '#3ecf8e', borderRadius:'4px' },
      };
    });
    const macroEmpty = macroRows.length===0;
    const macroNoneDefined = st.macroCategories.length===0;

    // ---- Scheda: Panoramica (quadro d'insieme: patrimonio, macro categorie, Da Assegnare, flusso) ----
    const overviewMonths = this.monthsInRange(6);
    const readyTrendVals = overviewMonths.map(mo => this.readyToAssignAt(mo));
    const rtMin = Math.min(...readyTrendVals), rtMax = Math.max(...readyTrendVals);
    const rtSpan = (rtMax-rtMin)||1;
    const rtPts = readyTrendVals.map((v,i)=>{ const x = overviewMonths.length>1 ? (i/(overviewMonths.length-1))*100 : 50; const y = 56 - ((v-rtMin)/rtSpan)*50 - 3; return [x,y]; });
    const rtLinePath = rtPts.map((p,i)=>(i===0?'M':'L')+' '+p[0].toFixed(2)+' '+p[1].toFixed(2)).join(' ');
    const rtAreaPath = rtLinePath + ' L 100 60 L 0 60 Z';
    const rtDots = rtPts.map((p,i)=>({ style:{ position:'absolute', left:p[0]+'%', top:(p[1]/60*100)+'%', width:'7px', height:'7px', marginLeft:'-3.5px', marginTop:'-3.5px', borderRadius:'50%', background:'#3ecf8e', border:'2px solid '+C.bg2 }, title: this.monthAbbr(overviewMonths[i])+': '+this.mask(this.fmtEur(readyTrendVals[i])) }));
    const readyTrend = { areaPath:rtAreaPath, linePath:rtLinePath, dots:rtDots, startLabel:this.monthAbbr(overviewMonths[0]), endLabel:this.monthAbbr(overviewMonths[overviewMonths.length-1]) };

    // Flusso del mese: barra segmentata al 100%. La larghezza è auto-consistente
    // (assegnato totale di ogni macro categoria + quota non ancora assegnata),
    // così i blocchi sommano sempre esattamente a 100% senza sforare — non è
    // agganciata rigidamente alle "entrate totali" del mese, che potrebbero non
    // coincidere esattamente con quanto assegnato (es. per riporti dai mesi prima).
    const totalAssignedThisMonth = Object.values(macroBuckets).reduce((s,b)=>s+b.assigned,0);
    const flowTotal = Math.max(0.01, totalAssignedThisMonth + Math.max(0, readyNow));
    const flowSegsList = Object.values(macroBuckets).filter(b=>b.assigned>0.005).map((b,i)=>({
      name:b.name, valueText:this.fmtEur(b.assigned),
      widthStyle:{ width:((b.assigned/flowTotal)*100)+'%', height:'100%', background: spendPalette[i%spendPalette.length] },
    }));
    const flowUnallocatedText = this.mask(this.fmtEur(Math.max(0,readyNow)));
    const flowUnallocatedWidthStyle = { width:((Math.max(0,readyNow)/flowTotal)*100)+'%', height:'100%', background:C.b2 };
    const flowHasData = flowTotal>0.015;
    ...
    const macroCategoryList = st.macroCategories.map(mc=>({ id:mc.id, name:mc.name, onEdit:()=>this.editMacroCategory(mc), onDelete:()=>this.delMacroCategory(mc) }));
    const macroCategoryOptions = st.macroCategories.map(mc=>({ value:mc.id, label:mc.name }));
    ...
    const modalTitles = { account: m.mode==='edit'?'Modifica conto':'Nuovo conto', transaction: m.mode==='edit'?'Modifica movimento':'Nuovo movimento', category: m.mode==='edit'?'Modifica categoria':'Nuova categoria', group: m.mode==='edit'?'Modifica gruppo':'Nuovo gruppo', macro: m.mode==='edit'?'Modifica macro categoria':'Nuova macro categoria', import:'Importa movimenti', move:'Sposta fondi' };
    ...
    return {
      ...
      dashSideItems, dashChips, isDashOverview, isDashNetWorth, isDashSpending, isDashIncomeExpense, isDashMacro, isDashGoals, isDashStruggling, isDashSavings,
      ...
      readyTrend, flowSegsList, flowUnallocatedText, flowUnallocatedWidthStyle, flowHasData,
      ...
      // categories
      incomeCats, expenseGroups, existingGroups, hiddenCats, macroCategoryList, macroCategoryOptions, onNewMacroCategory:()=>this.newMacroCategory(), incomeEmpty:incomeCats.length===0, expenseEmpty:expenseGroups.length===0,
      ...
      isAccountModal:m.kind==='account', isTransactionModal, isCategoryModal:m.kind==='category', isGroupModal:m.kind==='group', isMacroModal:m.kind==='macro',
