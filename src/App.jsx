...
class App extends React.Component {
  constructor(props){
    ...
    this.state = {
      ...
      dash: { tab:'overview', range:'6m', accountId:'', groupId:'', categoryId:'', customFrom:'', customTo:'', ieView:'flow' },
  ...
  createStoreFromDb(db, options = {}){
    ...
    const save = () => {
      ...
      self.setState({ syncStatus: 'saving' });
      saveChain = saveChain
        .then(async () => {
          ...
          self.setState({ syncStatus: 'idle' });
        })
        .catch((e) => {
          console.error('Salvataggio cloud non riuscito:', e);
          self.setState({ syncStatus: 'error' });
        });
    };
    self._retrySave = save;
    if (saveOnInit) save();
  ...
  retrySync(){
    if(this._retrySave) this._retrySave();
    else this.setState({ syncStatus:'idle' });
  }
  async logout(){
    try {
  ...
  // ---- KPI fissi Dashboard: confronto "pace-adjusted" e Da Assegnare del mese corrente ----
  // Il mese corrente è quasi sempre parziale: confrontarlo con un mese intero passato
  // farebbe sempre sembrare che si spenda pochissimo. Calcoliamo quindi, per ciascuno
  ...
  paceAdjustedAvg(type, accId){
    ...
    for(let i=1; i<=3; i++){
      ...
      const cutoff = Math.min(dayNum, daysInMo);
      const cutoffStr = String(cutoff).padStart(2,'0');
      let sum=0;
      this.state.transactions.forEach(t=>{
        if(t.type===type && (t.date||'').slice(0,7)===mo && (t.date||'').slice(8,10)<=cutoffStr && (!accId || t.accountId===accId)) sum += this.parseNum(t.amount);
      });
      sums.push(sum);
    }
    return sums.reduce((s,v)=>s+v,0) / (sums.length||1);
  }
  curMonthToDate(type, accId){
    const todayIso = this.today(); const curMo = todayIso.slice(0,7);
    let sum=0;
  ...
  // Come sopra ma "come era" a fine di un mese passato: serve alla Panoramica per il
  // trend del Da Assegnare, quindi calcola il saldo dei conti di budget FINO A quel
  // mese (non quello di oggi) invece di usare onBudgetCash(), che è sempre "adesso".
  onBudgetCashAt(mo){
    let bal = 0;
    this.state.accounts.filter(a=>a.onBudget!==false).forEach(a=>{
      bal += this.parseNum(a.initialBalance);
      this.state.transactions.forEach(t=>{
        if((t.date||'').slice(0,7) > mo) return;
        const amt = this.parseNum(t.amount);
        if(t.type==='Income' && t.accountId===a.id) bal += amt;
        else if(t.type==='Expense' && t.accountId===a.id) bal -= amt;
        else if(t.type==='Transfer'){ if(t.accountId===a.id) bal -= amt; if(t.toAccountId===a.id) bal += amt; }
      });
    });
    return bal;
  }
  readyToAssignAt(mo){
    const catList = this.state.categories.filter(c=>c.type==='Expense' && !c.hidden);
    const totalAvail = catList.reduce((s,c)=>s+this.catAvail(c.id, mo), 0);
    return this.onBudgetCashAt(mo) - totalAvail;
  }
  ...
  // Come catAvail, ma per PIÙ mesi in una volta sola: cammina in ordine cronologico
  // da earliestMonth() fino all'ultimo mese richiesto, accumulando il riporto via via,
  // invece di richiamare catCarryover() (che rifà tutto da capo) una volta per ogni
  // mese richiesto. Usato dai report che guardano un intero periodo (es. "Categorie
  // in affanno"): evita che il costo cresca col quadrato del numero di mesi.
  catAvailSeries(catId, months){
    const result = {};
    if(!months || !months.length) return result;
    const wanted = {}; months.forEach(m=>{ wanted[m]=true; });
    const lastWanted = months.reduce((a,b)=> b>a?b:a, months[0]);
    let mo = this.earliestMonth();
    let carry = 0, guard = 0;
    while(mo <= lastWanted && guard<1200){
      const avail = carry + this.catAssignedMonth(catId, mo) - this.catSpentMonth(catId, mo);
      if(wanted[mo]) result[mo] = avail;
      carry = Math.max(avail, 0);
      mo = this.addMonth(mo, 1);
      guard++;
    }
    return result;
  }
  ...
  setDashIeView(ieView){ this.setState({ dash: Object.assign({}, this.state.dash, { ieView }) }); }
  ...
  onFilterChange(e){ const k = e.target.dataset.filter; const filters = Object.assign({}, this.state.filters); filters[k] = e.target.value; this.setState({ filters, selectedTxnIds:{}, txnLimit:150 }); }
  clearFilters(){ this.setState({ filters:{ from:'', to:'', accountId:'', categoryId:'', type:'', search:'' }, selectedTxnIds:{}, txnLimit:150 }); }
  showMoreTxns(){ this.setState({ txnLimit: this.state.txnLimit + 150 }); }
  ...
  buildViewModel(){
    ...
    // I colori semantici (verde/rosso/ambra/accento) erano tarati sul tema scuro, dove
    // il contrasto è ottimo (~9-10:1 su sfondo scuro). Sullo stesso sfondo bianco/crema
    // del tema chiaro, però, scendevano a 2-3,2:1 — sotto la soglia di leggibilità
    // WCAG (4.5:1 per il testo normale). Uso quindi varianti più scure e sature per
    // il tema chiaro, mantenendo lo stesso significato (verde=positivo, rosso=negativo,
    // ambra=attenzione, blu=accento), ma leggibili su sfondo chiaro.
    const P = isLight
      ? { text:C.t0, muted:C.t1, dim:C.t2, accent:'#3d6fd6', green:'#0b7d45', red:'#d93025', amber:'#a35200' }
      : { text:C.t0, muted:C.t1, dim:C.t2, accent:'#5b8def', green:'#3ecf8e', red:'#f0616d', amber:'#e2b341' };
    ...
    // -- KPI (sempre visibili, indipendenti dalla scheda attiva e dal filtro Periodo:
    // sono sempre riferiti al mese corrente, confrontato "pace-adjusted" con la media
    // degli ultimi 3 mesi completi allo stesso giorno del mese) --
    ...
    const kpis = [
      ...
      { label:'Da assegnare', value:this.mask(this.fmtEur(readyNow)), sub: readyNow<-0.005?'sovra-assegnato':'aggiornato ad oggi', valColor: readyNow<-0.005?P.red:(readyNow>0.005?P.green:P.text) },
    ...
    const dashTabDefs = [
      ['overview','Panoramica'], ['networth','Patrimonio netto'], ['incomeexpense','Entrate vs Uscite'],
      ['spending','Spese per categoria'], ['macro','Macro categorie'],
    ...
    const isDashOverview = dash.tab==='overview';
    ...
    const isDashMacro = dash.tab==='macro';
    ...
    // ---- Ripartizione delle entrate per gruppo (= macro categoria), mese corrente ----
    // Un solo grafico, chiaro: quanto delle entrate REALI di questo mese è stato speso
    // in ciascun gruppo, in percentuale. Quando crei un gruppo "Risparmio" e ci sposti
    // dentro le categorie di risparmio/investimento, comparirà qui come una fetta
    // come tutte le altre — niente da configurare a parte.
    const shareBuckets = {};
    st.groups.forEach(g=>{ shareBuckets[g.name] = { id:g.name, name:g.name, spent:0 }; });
    shareBuckets[''] = { id:'', name:'Senza gruppo', spent:0 };
    expenseCatList.forEach(c=>{
      const key = (c.group && shareBuckets[c.group]) ? c.group : '';
      shareBuckets[key].spent += this.catSpentMonth(c.id, goalsMonth);
    });
    const incomeForShare = this.curMonthToDate('Income', null);
    const macroRowsRaw = Object.values(shareBuckets).filter(b=>b.spent>0.005).sort((a,b)=>b.spent-a.spent);
    const macroRows = macroRowsRaw.map((b,i) => {
      const pct = incomeForShare>0.005 ? (b.spent/incomeForShare)*100 : 0;
      const color = spendPalette[i%spendPalette.length];
      return {
        id:b.id, name:b.name, spentText:this.fmtEur(b.spent), pctText:Math.round(pct)+'%',
        dotStyle:{ width:'9px', height:'9px', borderRadius:'2px', background:color, flexShrink:0 },
        barStyle:{ width: Math.max(1,Math.min(100,pct))+'%', height:'100%', background:color, borderRadius:'4px' },
      };
    });
    const macroEmpty = macroRows.length===0;
    const macroNoneDefined = st.groups.length===0;
    const macroTotalSpent = macroRowsRaw.reduce((s,b)=>s+b.spent,0);
    const macroSpentPct = incomeForShare>0.005 ? Math.min(999,Math.round((macroTotalSpent/incomeForShare)*100)) : 0;
    const macroRemainderPct = Math.max(0, 100-macroSpentPct);
    const macroRemainderText = this.fmtEur(Math.max(0, incomeForShare-macroTotalSpent));
    const macroIncomeText = this.fmtEur(incomeForShare);
    const macroHasIncome = incomeForShare>0.005;
    ...
    const txnTotalCount = txns.length;
    const txnLimit = st.txnLimit || 150;
    const txnHasMore = txnTotalCount > txnLimit;
    const txnRemainingCount = Math.max(0, txnTotalCount - txnLimit);
    ...
    const onShowMoreTxns = ()=>this.showMoreTxns();
    ...
    return {
      ...
      syncStatus: st.syncStatus, onRetrySync:()=>this.retrySync(),
      ...
      dashSideItems, dashChips, isDashOverview, isDashNetWorth, isDashSpending, isDashIncomeExpense, isDashMacro, isDashSavings,
      dashIeView: dash.ieView, onSetIeFlow:()=>this.setDashIeView('flow'), onSetIeFixedVar:()=>this.setDashIeView('fixedvar'),
      ...
      ieBars, ieInsight,
      macroRows, macroEmpty, macroNoneDefined,
      macroIncomeText, macroHasIncome, macroSpentPct, macroRemainderText,
      ...
      txnHasMore, txnRemainingCount, onShowMoreTxns,
        ...
        <form onSubmit={(e)=> mode==='forgot' ? this.submitForgotPassword(e) : (mode==='reset' ? this.submitResetPassword(e) : this.submitLogin(e))} style={{width:'100%', maxWidth:'360px', background:C.bg2, border:'1px solid '+C.b1, borderRadius:'16px', padding:'28px'}}>
          ...
          {(mode==='login') ? (<React.Fragment>
            ...
            <div style={{textAlign:'right', marginBottom:'4px'}}>
              <button type="button" onClick={()=>this.showForgotPassword()} style={{background:'none', border:'none', color:(isLight?'#3d6fd6':'#5b8def'), fontSize:'12px', fontWeight:'600', cursor:'pointer', padding:'0'}}>Password dimenticata?</button>
            </div>
            ...
            {error ? (<div style={{marginTop:'10px', padding:'9px 12px', background:'rgba(240,97,109,0.1)', border:'1px solid rgba(240,97,109,0.3)', borderRadius:'9px', color:(isLight?'#d93025':'#f0616d'), fontSize:'12.5px', fontWeight:'600'}}>{error}</div>) : null}
          ...
          {(mode==='forgot') ? (<React.Fragment>
            ...
            {error ? (<div style={{marginTop:'4px', padding:'9px 12px', background:'rgba(240,97,109,0.1)', border:'1px solid rgba(240,97,109,0.3)', borderRadius:'9px', color:(isLight?'#d93025':'#f0616d'), fontSize:'12.5px', fontWeight:'600'}}>{error}</div>) : null}
            {info ? (<div style={{marginTop:'4px', padding:'9px 12px', background:'rgba(62,207,142,0.1)', border:'1px solid rgba(62,207,142,0.3)', borderRadius:'9px', color:(isLight?'#0b7d45':'#3ecf8e'), fontSize:'12.5px', fontWeight:'600'}}>{info}</div>) : null}
