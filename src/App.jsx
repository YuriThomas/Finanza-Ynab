...
class App extends React.Component {
  ...
  catCarryover(catId, month){
    let mo=this.earliestMonth(); let acc=0; let guard=0;
    while(mo < month && guard<600){ acc += this.catAssignedMonth(catId,mo) - this.catSpentMonth(catId,mo); mo=this.addMonth(mo,1); guard++; }
    return acc;
  }
  ...
  resetData(){ if(window.confirm('Ripristinare i dati d’esempio? Le tue modifiche andranno perse.')){ this.store.reset(); this.setState({ month:'2026-07', filters:{from:'',to:'',accountId:'',categoryId:'',type:'',search:''} }); this.refresh(); } }
  ...
  clearFilters(){ this.setState({ filters:{ from:'', to:'', accountId:'', categoryId:'', type:'', search:'' } }); }
  ...
  buildViewModel(){
    ...
    const totalAvailableInCategories = expenseCatList.reduce((sum, c) => {
      const assigned = budgetMap[c.id] || 0;
      const spent = monthTxns
        .filter(t => t.type === 'Expense' && t.categoryId === c.id)
        .reduce((s, t) => s + t.amount, 0);
      const carry = this.catCarryover(c.id, st.month);
      return sum + carry + assigned - spent;
    }, 0);
    const ready = this.onBudgetCash() - totalAvailableInCategories;
    const readyPositive = ready >= -0.005;
    const readyWarning = !readyPositive;
    const readyCardStyle = { padding:'10px 16px', borderRadius:'12px', textAlign:'right', minWidth:'170px', maxWidth:'270px', border:'1px solid', borderWidth: readyWarning?'2px':'1px', background: readyPositive?'rgba(62,207,142,0.09)':'rgba(240,97,109,0.13)', borderColor: readyPositive?'rgba(62,207,142,0.3)':'rgba(240,97,109,0.6)', color: readyPositive?P.green:P.red, boxShadow: readyWarning?'0 0 0 3px rgba(240,97,109,0.14)':'none' };
    ...
    return {
      ...
      readyCardStyle, readyLabel, readyText, readyWarning, readySubtext, budgetGroups, budgetEmpty: expenseCatList.length===0,
      // transactions
