...
class App extends React.Component {
  ...
  parseNum(v){ if(typeof v === 'number') return v; return parseFloat(String(v||'').replace(/\s/g,'').replace('.','').replace(',','.')) || parseFloat(String(v||'').replace(',','.')) || 0; }
  ...
  buildViewModel(){
    ...
    // Tema chiaro: il file non ha una palette centralizzata (colori scritti singolarmente
    // in centinaia di punti), quindi il tema chiaro si ottiene invertendo i colori
    // dell'intera app via filtro CSS, invece di riscrivere ogni colore a mano.
    const isLight = st.theme === 'light';
    const rootFilterStyle = isLight ? { filter:'invert(1) hue-rotate(180deg)' } : {};
    ...
    const budgetGroups = groupNames.map(g => {
      ...
      return { name: g||'Senza gruppo', rows, assignedText:this.fmtEur(gAssigned), spentText:this.fmtEur(gSpent), availText:this.fmtEur(gAvail), availStyle:{ fontFamily:"'JetBrains Mono',monospace", fontWeight:600, fontSize:'11.5px', color:gAvailColor }, isEmpty: rows.length===0, collapsed, onToggleCollapse:()=>this.toggleGroupCollapse(g), chevronStyle:{ width:'14px', height:'14px', flexShrink:'0', transition:'transform .15s', transform: collapsed?'rotate(-90deg)':'rotate(0deg)', cursor:'pointer', color:'#6b7079' } };
    }).filter(gr => !(gr.isEmpty && gr.name==='Senza gruppo'));
    // Pronto per assegnare = liquidità nei conti di budget − tutto ciò che è stato
    // assegnato alle categorie fino ad oggi (senza nettare lo speso: uno sforamento
    // di categoria NON deve mai far salire il Pronto per assegnare sopra la liquidità
    // reale). È l'invariante YNAB: assegnato totale + Pronto per assegnare = liquidità.
    // Pronto per assegnare = liquidità nei conti di budget − SOLO quanto assegnato
    // nel mese selezionato. Nessun dato storico, obiettivo, spesa o disponibilità
    // negativa di categoria entra in questo calcolo: è isolato al mese corrente.
    const totalAssignedThisMonth = expenseCatList.reduce((sum, c) => sum + (budgetMap[c.id] || 0), 0);
    const ready = this.onBudgetCash() - totalAssignedThisMonth;
    ...
    const dashRangeBtns = [['3m','3M'],['6m','6M'],['12m','1A'],['ytd','YTD'],['all','Tutto'],['custom','Personalizzato']].map(x => ({
      ...
      style:{ padding:'6px 12px', border:'none', borderRadius:'7px', fontSize:'12.5px', fontWeight:600, cursor:'pointer', background: dash.range===x[0]?'#5b8def':'transparent', color: dash.range===x[0]?'#fff':'#9297a1', transition:'all .12s' }
    }));
    ...
    return {
      ...
      isLight, rootFilterStyle, onToggleTheme:()=>this.toggleTheme(),
      ...
      <div style={{minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0c0d10', color:'#e7e9ee', fontFamily:"'Hanken Grotesk',system-ui,-apple-system,sans-serif", padding:'20px'}}>
        <form onSubmit={(e)=>this.submitLogin(e)} style={{width:'100%', maxWidth:'360px', background:'#101216', border:'1px solid #1c1f26', borderRadius:'16px', padding:'28px'}}>
          ...
          <label style={{display:'flex', flexDirection:'column', gap:'6px', fontSize:'12px', color:'#9297a1', fontWeight:'600', marginBottom:'6px'}}>
            Password
            <input type="password" value={password} onChange={(e)=>this.onAuthPassword(e)} style={{padding:'10px 12px', background:'#0c0d10', border:'1px solid #23262d', borderRadius:'9px', color:'#e7e9ee', fontSize:'14px'}} />
          </label>
